...
(function(root){
  ...
  function loadRuntimeLock(){try{if(document.querySelector('script[data-techops-production-runtime-lock]')){loadDirector();return;}if(!appendScript('data-techops-production-runtime-lock','production_runtime_lock.js',loadDirector,function(){root.__productionRuntimeLockWireError='production_runtime_lock.js';loadDirector();}))loadDirector();}catch(e){root.__productionRuntimeLockWireError=String(e&&e.stack||e);loadDirector();}}
  function boot(){try{if(document.querySelector('script[data-techops-production-bootstrap]')){loadRuntimeLock();return;}snapshotParserChain();if(!appendScript('data-techops-production-bootstrap','production_bootstrap.js',loadRuntimeLock,function(){root.__productionBootstrapWireError='production_bootstrap.js';loadDirector();}))loadDirector();}catch(e){root.__productionBootstrapWireError=String(e&&e.stack||e);loadDirector();}}
