...
function draw() {
  ...
  if (!draw._tlCv) {
    ...
    draw._tlMap = null; draw._tlQ = -1; draw._tlConv = [];
  }
  ...
  if (draw._tlMap !== s.map || draw._tlQ !== tlQ) {
    ...
    pxCtx = tc;
    draw._tlConv = [];
    for (let y = 0; y < MAPH; y++) { const row = s.map[y]; if (!row) continue; for (let x = 0; x < MAPW; x++) { const t = row[x] || 0; if (t === 5) { draw._tlConv.push([x, y]); continue; } drawTile(t, x, y, tm); } }
    pxCtx = null;
    draw._tlMap = s.map; draw._tlQ = tlQ;
  }
