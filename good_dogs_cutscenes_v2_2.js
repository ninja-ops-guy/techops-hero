/* TechOps Hero — Good Dogs pre-rendered cutscene player v3.0.
 * iPhone contract:
 *   - never expose a black video surface while media is preparing
 *   - one deliberate user gesture starts playback
...
(function(){
  ...
  function makeOverlay(){
    ...
    root.innerHTML='<div class="gd-film-frame"><div class="gd-film-poster" aria-hidden="true"></div><div class="gd-film-title">GOOD DOGS PROTOCOL</div><video class="gd-film-video" playsinline webkit-playsinline preload="metadata" muted></video><div class="gd-film-status" aria-live="polite">PREPARING TRANSMISSION…</div><button class="gd-film-play" type="button">PLAY CUTSCENE</button><div class="gd-crt" aria-hidden="true"></div><div class="gd-vignette" aria-hidden="true"></div><button class="gd-film-skip" type="button">SKIP</button></div>';
  ...
  async function play(id,options={}){
    ...
    video.controls=false;video.muted=true;video.defaultMuted=true;video.volume=0;video.setAttribute("muted","");video.setAttribute("playsinline","");video.setAttribute("webkit-playsinline","");video.preload=ios?"metadata":"auto";skip.disabled=false;skip.textContent="SKIP";
    let done=false,startTimer=0,stallTimer=0,hardTimer=0,lastTime=-1,progressed=false,retryReason="",playInFlight=false,frameRevealed=false;
    return new Promise(resolve=>{
      ...
      video.onended=()=>finish(STATUS.COMPLETED,"ended");video.onerror=()=>waitForUser("media-error","VIDEO UNAVAILABLE · RETRY OR SKIP");video.onloadedmetadata=()=>{if(ios)waitForUser("ios-ready","VIDEO READY · PLAY CUTSCENE");};video.onloadeddata=()=>{if(!ios&&!progressed)attemptPlay();};video.oncanplay=()=>{if(ios&&!progressed)waitForUser("ios-ready","VIDEO READY · PLAY CUTSCENE");};video.onplaying=()=>{clearTimeout(startTimer);markPlaying();};video.ontimeupdate=()=>{const t=Number(video.currentTime||0);if(t>lastTime+.01){lastTime=t;if(t>.02){progressed=true;requestDecodedFrame();markPlaying();}}};video.onwaiting=video.onstalled=()=>{if(progressed)armStall();};
      video.src=options.src||def.src;try{video.load();}catch(_){}
      if(ios){clearTimeout(startTimer);startTimer=setTimeout(()=>waitForUser("ios-ready-timeout","PLAY CUTSCENE"),1800);}else{attemptPlay();}
    });
  }
  window.GoodDogsCutscenes={VERSION:"3.0",STATUS,play,clips:CLIPS,state:ensureState,isIOSDevice};
})();