/* TechOps Hero — Good Dogs pre-rendered cutscene player
   v2.5: silent source-detail presentation, non-destructive retro treatment,
   state migration across the title-screen -> campaign handoff, and resilient
   pointer/touch exit handling for iPhone/WebKit.
*/
(function(){
  "use strict";
  const CLIPS={
  ...
  function makeOverlay(){
    ...
    root.innerHTML='<div class="gd-film-frame"><div class="gd-film-title">GOOD DOGS PROTOCOL</div><video class="gd-film-video" playsinline webkit-playsinline preload="auto" muted></video><div class="gd-crt" aria-hidden="true"></div><div class="gd-vignette" aria-hidden="true"></div><button class="gd-film-skip" type="button">SKIP</button></div>';
  ...
  const css=document.createElement("style");css.textContent=`
    ...
    @media(max-width:600px){.gd-film-frame{width:100vw;height:100vh;border:0}.gd-film-title,.gd-film-skip{top:max(8px,env(safe-area-inset-top))}.gd-film-skip{right:max(10px,env(safe-area-inset-right));min-width:86px;min-height:48px}}
  `;document.head.appendChild(css);
  async function play(id,options={}){
    const def=CLIPS[id];if(!def)throw new Error("Unknown Good Dogs cutscene: "+id);
    const root=makeOverlay(),video=root.querySelector(".gd-film-video"),skip=root.querySelector(".gd-film-skip"),state=ensureState(),priorOverflow=document.body.style.overflow;
    ...
    return new Promise((resolve)=>{
      const finish=(skipped,source)=>{
        ...
        window.__goodDogsCutsceneExit={id,skipped:!!skipped,source:source||"unknown",at:Date.now()};
        try{video.pause();video.removeAttribute("src");video.load();}catch(_){}
        root.classList.remove("active");delete root.dataset.activeCutscene;document.body.style.overflow=priorOverflow;
        state[id]={seen:true,skipped:!!skipped,at:Date.now()};window.__goodDogsCutsceneState[id]=state[id];
        if(typeof options.onStateWrite==="function")options.onStateWrite({id,skipped:!!skipped});cleanup();resolve({id,skipped:!!skipped});
      };
      const isSkipTarget=(target)=>{try{return !!(target&&target.closest&&target.closest(".gd-film-skip"));}catch(_){return false;}};
      const skipEvent=(e)=>{if(!isSkipTarget(e&&e.target)&&e&&e.currentTarget!==skip)return;try{e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();}catch(_){}skip.disabled=true;skip.textContent="SKIPPING…";finish(true,e&&e.type||"skip");};
      const rootCapture=(e)=>{if(isSkipTarget(e&&e.target))skipEvent(e);};
      const key=(e)=>{if(e.key==="Escape"||e.key==="Enter")finish(true,"keyboard");};
      const cleanup=()=>{
      ...
      video.onended=()=>finish(false,"ended");video.onerror=()=>finish(true,"media-error");document.addEventListener("keydown",key);
      video.src=options.src||def.src;video.currentTime=0;video.play().catch(()=>{video.controls=true;window.__goodDogsCutsceneAutoplayBlocked={id,at:Date.now()};});
    });
  }
  window.GoodDogsCutscenes={VERSION:"2.5",play,clips:CLIPS,state:ensureState};
})();
