/* TechOps Hero — Good Dogs pre-rendered cutscene player v3.3.
...
(function(){
  ...
  const css=document.createElement("style");css.textContent=`
    ...
    .gd-film-status{display:none!important;position:absolute;left:50%;top:50%;transform:translate(-50%,-72px);z-index:6;color:#9be8cf;font:700 11px/1.4 monospace;letter-spacing:1.2px;text-align:center;width:min(88vw,560px);pointer-events:none;text-shadow:0 0 12px #6ee7c766}
  ...
  async function play(id,options={}){
    ...
    return new Promise(resolve=>{
      ...
      const waitForUser=(reason,label)=>{if(done)return;retryReason=reason;playInFlight=false;clearTimers();frame.classList.remove("gd-playing");status.style.display="none";status.textContent="";playBtn.textContent=reason==="media-error"?"RETRY VIDEO":"PLAY CUTSCENE";playBtn.classList.add("active");window.__goodDogsCutsceneNeedsGesture={id,reason,label,at:Date.now(),currentTime:Number(video.currentTime||0),readyState:Number(video.readyState||0),ios};};
      ...
      const armStall=()=>{clearTimeout(stallTimer);stallTimer=setTimeout(()=>waitForUser("media-stall","VIDEO PAUSED · PLAY TO RESUME"),4200);};
      const markPlaying=()=>{if(done)return;progressed=true;playInFlight=false;playBtn.classList.remove("active");status.style.display="none";status.textContent="";frame.classList.add("gd-playing");requestDecodedFrame();armStall();clearTimeout(hardTimer);hardTimer=setTimeout(()=>waitForUser("hard-timeout","VIDEO PAUSED · PLAY TO RESUME"),60000);};
  ...
  window.GoodDogsCutscenes={VERSION:"3.3",STATUS,play,clips:CLIPS,state:ensureState,isIOSDevice};
})();
