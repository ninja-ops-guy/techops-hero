...
test('house interaction remains delegated instead of becoming a punch',()=>{
 const f=fixture(true);let calls=0;f.r.interact=()=>calls++;
 f.r.TechOpsNightRuntime={blocked:()=>false,atHome:()=>true};
 assert.equal(f.controls.contextAction(),'home');assert.equal(f.controls.dispatch('punch'),true);
 assert.equal(calls,1);assert.equal(f.n._nightCombat,undefined);assert.equal(f.e.hp,500);
});
test('Charger interaction stays reachable and a nearby enemy keeps punch ownership',()=>{
 const f=fixture(true);let calls=0;f.r.interact=()=>calls++;f.n.enemies=[];
 assert.equal(f.controls.contextAction(),'charger');f.controls.dispatch('punch');assert.equal(calls,1);
 f.n.enemies=[f.e];assert.equal(f.controls.contextAction(),null);f.controls.dispatch('punch');advance(f,80);
 assert.equal(calls,1);assert.ok(f.e.hp<500);
});
test('pause/game-over/hidden tab and director lock refuse both direct and input actions',()=>{
 for(const mode of ['paused','gameOver','hidden','director']){
  const f=fixture(true);
  if(mode==='hidden')f.r.document={hidden:true};else if(mode==='director')f.r.TechOpsNightRuntime={blocked:()=>true};else f.r.S[mode]=true;
  assert.equal(f.controls.ready(),false);assert.equal(f.controls.dispatch('kick'),false);
  assert.equal(f.api.kick(f.n,{}),false);assert.equal(f.e.hp,500);
 }
});
test('canonical Night world wins over a stale global alias',()=>{
 const f=fixture(true),live=f.n;f.r.S.nightMode=live;f.r.NM={...live,_v736:{}};
 assert.equal(f.controls.owns(),true);f.controls.dispatch('kick');advance(f,120);assert.ok(f.e.hp<500);
});
test('audio failures cannot interrupt contact or KO settlement',()=>{
 const f=fixture();f.e.hp=14;f.r.sfx=()=>{throw Error('audio device unavailable');};
 hit(f,{},'punch');assert.equal(f.e.alive,false);assert.equal(f.n.cash,10);assert.equal(f.n.kills,1);
});
