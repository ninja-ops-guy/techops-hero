/* GOOD BOYS production mechanics — paired-combat authority v2.
...
(function(root){
  ...
  var VERSION=2,state={airDashes:0,boostUsed:false,lastGround:false,comboStep:0,lastAttackAt:0,lastThrowAt:0};
  ...
  function nearestEnemy(max){
    try{var n=root.NM,arr=n&&n.enemies;if(!n||!arr)return null;var best=null,bd=max||150;for(var i=0;i<arr.length;i++){var e=arr[i];if(!e||e.alive===false||Number(e.hp)<=0)continue;var d=Math.abs(enemyCenter(e)-((Number(n.x)||0)+(Number(n.w)||22)/2));if(d<bd){bd=d;best=e;}}return best;}catch(e){return null;}
  }
  ...
  function activeStrike(){
    try{if(typeof root.nmJab==="function"){root.nmJab();return true;}if(typeof root.interact==="function"){root.interact();return true;}root.dispatchEvent(new KeyboardEvent("keydown",{key:"e",bubbles:true,cancelable:true}));return true;}catch(e){return false;}
  }
  ...
  function pairedAttack(){
    try{var c=cs(),n=root.NM;if(!c||!n||n.block)return false;var t=now();if(t-state.lastAttackAt>720)state.comboStep=0;state.lastAttackAt=t;state.comboStep=(state.comboStep%3)+1;activeStrike();if(state.comboStep>=2)partnerAssist(state.comboStep);c.sync=Math.min(100,(c.sync||0)+(state.comboStep===3?6:2));msg(state.comboStep===1?"🐾 1 · LEAD STRIKE":state.comboStep===2?"🐾 2 · PARTNER FOLLOW":"🐾 3 · TANDEM LAUNCH");return true;}catch(e){return false;}
  }
  ...
  root.TechOpsGoodBoysReferenceMechanics={VERSION:VERSION,boostJump:boostJump,airDash:airDash,pairedAttack:pairedAttack,partnerThrow:partnerThrow,midAirCatch:midAirCatch,throwOrCatch:throwOrCatch,nearPartner:nearPartner,state:state,tick:tick,timer:timer};
