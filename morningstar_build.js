...
(function(root){
  ...
  var VERSION=3, wrappedResolve=false, wrappedSetup=false, wrappedNight=false, injectedDay=-1;
  var PHASES=[
  ...
  function completed(s,id){return !!(s.story&&Array.isArray(s.story.completedActs)&&s.story.completedActs.indexOf(id)>=0);}
  function storyStatus(s,id){try{return root.TechOpsStory&&root.TechOpsStory.transitionStatus?root.TechOpsStory.transitionStatus(s,id):null;}catch(e){root.__morningstarStorySyncError=String(e&&e.stack||e);return null;}}
  function eligible(s){var f=facts(s);return !!(completed(s,"act_4")&&f.felicia_alliance&&f.morningstar_hangar_revealed);}
  function capabilityUnlocked(minPhase,s){s=s||campaign();return !!(s&&eligible(s)&&current(s)>=Math.max(0,Number(minPhase)||0));}
  ...
  function canAdvance(s){s=s||campaign();var p=s&&activePhase(s),status=s&&current(s)===3?storyStatus(s,"act_5"):null;return !!(s&&p&&eligible(s)&&dayComplete(s,p)&&nightComplete(s,p)&&(!status||status.eligible||status.completed));}
  function applyStoryFacts(s,phaseIndex){if(phaseIndex>=5)facts(s).watchdog_configured=true;}
  function syncStoryAct(s,phaseIndex){try{if(phaseIndex<4||!root.TechOpsStory||typeof root.TechOpsStory.completeAct!=="function")return false;if(completed(s,"act_5"))return true;var status=storyStatus(s,"act_5");if(!status||!status.eligible)return false;root.TechOpsStory.completeAct(s,"act_5");return true;}catch(e){root.__morningstarStorySyncError=String(e&&e.stack||e);return false;}}
  function reconcileStory(s){if(!s||current(s)<4||completed(s,"act_5"))return false;if(!syncStoryAct(s,current(s)))return false;save(s);return true;}
  function advance(){
    var s=campaign();if(!s||!canAdvance(s))return{success:false,reason:"requirements_not_met"};
    var st=store(s),idx=current(s),p=PHASES[idx],next=idx+1;
    if(next===4&&!syncStoryAct(s,next))return{success:false,reason:"story_transition_not_eligible"};
    st.phase=next;
  ...
  function openHub(){var s=campaign();if(!s||!eligible(s)||typeof root.dlg!=="function")return false;var r=requirements(s);if(!r)return root.dlg("MORNINGSTAR // COMPLETE","All five build phases are complete. Authority remains distributed and inspectable.",[{t:"Close",f:root.closeDlg}]);var body="<b>"+r.name+"</b><br><br>DAY WORK<br>"+r.dayTickets.map(function(t){return(t.complete?"✓ ":"□ ")+t.label;}).join("<br>")+"<br><br>NIGHT RECOVERY<br>"+(r.night.complete?"✓ ":"□ ")+r.night.district.toUpperCase()+" // STREET "+(r.night.stage+1)+"<br><br>UNLOCKS<br>"+r.unlocks.join(" · ");var opts=[{t:"Close",f:root.closeDlg}];if(root.TechOpsSwarmDoctrine&&current(s)>=3)opts.unshift({t:"Open Swarm Log",f:function(){root.TechOpsSwarmDoctrine.openLog();}});root.dlg("MORNINGSTAR BUILD // "+(r.phase+1)+"/5",body,opts);return true;}
  function snapshot(){var s=campaign();if(!s)return null;var st=store(s),p=activePhase(s);return{version:VERSION,eligible:eligible(s),currentPhase:current(s),currentPhaseName:p?p.name:"Complete",totalPhases:PHASES.length,unlocks:st.unlocks.slice(),completedDayTickets:st.completedDayTickets.slice(),nightRecoveredItems:st.nightRecoveredItems.slice(),canAdvance:canAdvance(s),requirements:requirements(s)};}
  function install(){var s=campaign();if(s)reconcileStory(s);installResolveHook();installSetupHook();installNightHook();injectPhaseTicket();return true;}
  root.TechOpsMORNINGSTARBuild={VERSION:VERSION,PHASES:PHASES,getCurrentPhase:function(){var s=campaign();return s?current(s):0;},getPhase:function(i){return PHASES[i]||null;},capabilityUnlocked:capabilityUnlocked,canAdvance:function(){var s=campaign();return !!(s&&canAdvance(s));},advance:advance,onTicketResolved:onTicketResolved,onNightRecovery:onNightRecovery,openHub:openHub,snapshot:snapshot,reconcileStory:reconcileStory,install:install};
  install();
