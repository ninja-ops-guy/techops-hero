...
| `ui_coop.js` + `ui_coop_p1–p15.js` | Online co-op/lobby UI kit (36 icons, 6×6 webp sheet: host/join, slots, mic, ping, pads, avatars) — loaded, reserved for the multiplayer cycle (no netcode in scope) |
| `v737_hooks.js` + `mike_actions.js/.atlas.js` | v7.37: third shift — Ghost Shift ch.1 AFTER HOURS (evidence-drive recovery, grade-A/partial outcomes, gs1 cinematic) and ch.2 DEAD DROPS (erased-report fragments → the distorted radio), plus the first mike_actions wiring (EOD style-rank celebration pose) |
| `v736_hooks.js` + `felicia_music`/`dogs_action`/`warden_null`/`portraits_ui`/`enemy_roster` payloads | v7.36: full wiring — Felicia's violin atlas drives the rooftop signal + den encore, dogs' action poses (arrival barks), portraits_ui dialog busts, WARDEN NULL fragment boss (Industrial, post-Ghost-Fork), enemy_roster frames on all five night archetypes, shuttle-crew alt title, the last Waldo quests (House Call / Family Bandwidth / The Missing Bird), night STYLE RANK D→S |
| `v735_hooks.js` + `dogs`/`ui_kit`/`k_action`/`bg_garage`/`bg_waldo_den` payloads | v7.35: good dogs — Manchez & Katrin render from their own atlas, nameplate dialogue portraits (K/Waldo/dogs), ORPHEUS-eye & NOC key art placed, Waldo garage & den interiors, the questline completes (Parts Run / Nowhere to Be / Party in the Stratosphere / Brothers Under One Sky → WALDO ASSIST) |
| `v734_hooks.js` + `k_studio`/`null_shepherd`/`ui_kit`/`bg_*` payloads | v7.34: ghost fork — six-mission K side story (Cell 118, the Ghost Fork, Cell 1984, the Archive choice, the Impossible Exit) on the v7.25 engine, NULL SHEPHERD night boss (teleport/beam/phase shift) from his own atlas, painted district backdrops in the night renderer, K companion (porch piano + fork decoy assist), Manchez & Katrin |
...
## v7.37 — Third Shift (Ghost Shift chapters 1–2 + first mike_actions wiring)

The v7.33 encrypted message becomes playable. **CH.1 — AFTER HOURS**: with the Ghost Shift seed, Industrial street 1 (the closed recycling plant) hides 3 evidence drives under a two-objective brief — clearing the street is one thing, recovering every drive before leaving is the grade. Full recovery pays $60, files `after-hours-grade-A` into the intel archive, and plays the **gs1** cinematic (GHOST SHIFT I — every drive was already wiped; they wanted the HARDWARE; the crescent-cursor calling card tags the wall); a partial clear files `after-hours-partial` and says so honestly. Drives reset per attempt. **CH.2 — DEAD DROPS**: one glowing cache per combat district (street 1, first visit each) files an erased incident-report fragment into the v7.33 docs; at three, the distorted radio speaks — "You fix their systems. I remember what they erased." (`_v737ch2`). **mike_actions, first honest wiring**: nights that end with style rank A/S close the EOD screen with Mike's celebration pose from the sheet (the payload Image is warmed at load so the first EOD never misses). Flags `_v737ch1/_v737ch2/_v737drops` persist in `S.meta`. Ghost Shift chapters 1–2 are playable; chapters 3–5 wait on the Echo sprite sheet (documented, top of the wish list). Also integrated this cycle: the **ui_coop** lobby kit payload wave (15/15 parts + assembler, real webp payload in-repo) is now loaded by `index.html`; `manchez_katrin_hits` (4/13 parts landed) stays unwired until the remaining parts arrive, and `waldo_b.atlas.js`/`waldo_ui.atlas.js` remain orphan metadata pending their payloads.

## v7.36 — Full Wiring (every delivered asset gets a job)

Everything the art drops delivered now works in-game. **Felicia's music atlas** drives the rooftop-signal close-up and a den-party encore cameo (B/TRUE endings — canon). **The dogs act**: Manchez barks your arrival, both cycle action poses at Waldo's. **portraits_ui** gives Felicia/Mike/cast dialogs proper busts (yielding to the v7.7 path). **WARDEN NULL** — the fragment that escaped into K's archive — resurfaces as the Industrial street-2 boss once the Shepherd is down: laser eyes, glitch step, and a prison-cage root, drawn from his 24-pose atlas. **enemy_roster** puts real frames on all five night archetypes over their silhouettes. **Alt title screen**: finishing Ghost Fork durably swaps the title art to the shuttle crew. **The Waldo questline is complete**: HOUSE CALL (the compromised set-top box before the party — choice persisted, TELEMETRY LINK), FAMILY BANDWIDTH (help his cousin's block without treating home like a scene — LOW-ORBIT RELAY, family intel rate), THE MISSING BIRD (BIRD-7 heard something about AeroTech, CRAWLER, and her — RECOVER/BURN/REDIRECT persisted, THREAT FORECAST). **Night STYLE RANK** (D→S: move variety, perfects, airtime, damage avoided) rides the HUD and the ride-home toast. Reserved for future cycles (sliced, delivered, deliberately unwired): `ui_lobby` (multiplayer lobby icons — no netcode in scope), `mike_actions` and `interiors` (combat-depth & Hollow Network cycles).

## v7.35 — Good Dogs (wiring & polish; the Waldo questline completes)

Manchez and Katrin render from their own 32-frame atlas (amber tactical / blue linguistic, collars glowing) everywhere they appear; the Ghost Fork decoy uses K's action atlas. **Dialogue chrome**: K, Waldo and the dogs get their nameplate portraits in DOM dialogs (cropped from the UI kit, cached, yielding to the v7.7 portrait path). **Key art placed**: the ORPHEUS-eye painting closes ORPHEUS WAKES, the NOC painting anchors the 03:17 emerald wake, and Waldo's **garage and den are real interiors** (painted backdrop swaps; the garage opens after the tracker quest). The **questline completes**: PARTS RUN (route choice persisted, car mod installed), NOWHERE TO BE (the no-objective drive — -25 stress and a restored 08:30 morning block, exactly once), PARTY IN THE STRATOSPHERE (den, choice persisted, the corner-listener catches a transmitting guest), and the tier-5 finale BROTHERS UNDER ONE SKY — **WALDO ASSIST: CLOSE AIR SUPPORT** (once per night at critical HP: +20 HP, the room floors) plus the shared intel archive. Canon patch folded in: K carries no transferred Mike memories (the K-0 crystal is recorded simulation data; INSTALL not RESTORE), K knows Mike through stories, Waldo & Mike are childhood friends, and the Vault escape ends aboard the shuttle, course Earth.

## v7.34 — Ghost Fork (K's side story, NULL SHEPHERD, painted districts)

The dimensional past opens. After the Ghost Shift seed (v7.33), six day-end missions on the shared cinematic engine tell **K: Ghost Fork** — TWO GOOD DOGS (Manchez & Katrin cross into the Penumbra Vault), THE MAN IN CELL 118 (K freed; the collar breaks), GHOST IN THE SYSTEM (the prison predicts Mike; it cannot predict a clone taking orders from two alien-AI dogs — GHOST FORK unlocked), CELL 1984 (the Waldo reunion — UNKNOWN VARIABLE → BROTHERS BY CHOICE, +20 rep), THE ARCHIVE OF MIKES (RESTORE / DESTROY / DIVIDE persisted in `_v734archive`; Divide is canon), and THE IMPOSSIBLE EXIT (four minds, one portal — "Just K."). **NULL SHEPHERD**, Warden Null's champion, becomes a real night boss on Suburbs street 2 — glitch-step teleports, a telegraphed satellite beam, a phase shift at half health — drawn from his own 95-frame generated atlas. After the finale, **K joins as a companion**: he plays the emerald piano at Waldo's porch (a real conversation, once per night), and GHOST FORK fires once per night at low HP — a decoy pulls all aggro for six seconds. The night renderer gains **painted district backdrops** (downtown, Long Wharf docks, industrial, Wooster Square, airport hangar, suburbs portal) — the v7.31 procedural street remains as the live fallback. Payload contract unchanged: sprites/backdrops inline in the platform deploy, loader stubs + procedural fallbacks in the repo.
