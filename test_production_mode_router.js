...
const context={console,setInterval(){return 1;},clearInterval(){},setTimeout(fn){q.push(fn);return q.length;},isFinite,
  ...
  TechOpsGoodBoysGameplayLoop:{tick(){}},TechOpsGoodDogsProduction:{tick(){}},TechOpsGoodBoysReferenceMechanics:{tick(){}},TechOpsGoodBoysCanon:{tick(){}},TechOpsNightReferenceVisuals:{install(){}}
};
context.globalThis=context;vm.createContext(context);vm.runInContext(src,context,{filename:"production_mode_router.js"});
const api=context.TechOpsProductionModeRouter;assert.ok(api);assert.strictEqual(api.VERSION,2);
api.setDesired("nightcrawler");api.launchNightCrawler();flush();
assert.ok(context.S.nightMode,"Night Crawler launch must end in night mode");assert.ok(context.NM,"Night Crawler launch must create Night world");assert.strictEqual(context.S.meta._char,"nightcrawler");assert.strictEqual(context.localStorage.getItem("techops_char"),"nightcrawler");assert.ok(enterCalls>=1);

context.S={nightMode:false,meta:{_v736:{m:6}}};context.NM=null;api.launchGoodBoys();flush();
assert.ok(context.S.nightMode,"Good Boys resume must enter Night engine");assert.ok(api.pairReady(),"Good Boys resume must attach Katrin and Manchez");assert.ok(v736Calls>=1);assert.strictEqual(context.localStorage.getItem("techops_char"),null,"Good Boys cannot inherit Night Crawler character selection");
...
console.log("Production mode router v2 lexical-runtime regression: PASS");
