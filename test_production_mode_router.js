...
const api=context.TechOpsProductionModeRouter;assert.ok(api);assert.strictEqual(api.VERSION,4);
...
const raw=src.toUpperCase();assert.ok(raw.includes("NIGHT CRAWLER"));assert.ok(raw.includes("118/1984"));assert.ok(raw.includes("GOOD_BOYS_PAIR_TIMEOUT"));assert.ok(raw.includes("CLEARBLOCKINGDIALOG"));assert.ok(src.includes('typeof S!=="undefined"'));assert.ok(src.includes('typeof NM!=="undefined"'));assert.ok(!src.includes('return root.S||null'));assert.ok(!src.includes('root.NM&&'));
console.log("Production mode router v4 input/coop regression: PASS");
