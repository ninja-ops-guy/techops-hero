...
elements["btn-start"].disabled=true;
elements["btn-start"].click=function(){if(this.disabled)return;assert.strictEqual(context.__productionTitleInternalStart,"nightcrawler","router must declare its one-shot nested title start");context.S={nightMode:false,clock:540,meta:{},inDialog:true};elements.dialogue.classList.remove("hidden");};
...
assert.strictEqual(context.__productionTitleInternalStart,null,"nested title bypass cannot leak beyond the canonical click");
...
assert.strictEqual(context.__techopsAlternateStartMode,"nightcrawler","the start handler must see alternate-mode ownership before state creation");
