...
n=world(2);api.configure('local');n.x=1200;n._v736.partner.x=969;steps(n,100);assert.equal(api.complete(2),false);assert.equal(api.interact(1),true);steps(n,100);assert.equal(api.complete(2),false);n._gbShipRevealed=true;steps(n,65);assert.equal(api.complete(2),true);assert.equal(api.complete(3),true);
