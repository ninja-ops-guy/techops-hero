...
(function(root){
  ...
  function sound(id){try{if(typeof root.sfx==='function')root.sfx(id);}catch(_){/* Cosmetic audio cannot abort a hit or reward. */}}
  function blocked(){const s=typeof S!=='undefined'?S:root.S;return !!(s&&(s.inDialog||s.inBattle||s.paused||s.gameOver)||root.document&&root.document.hidden||root.TechOpsNightRuntime&&root.TechOpsNightRuntime.blocked());}
  ...
  function attack(n,k={},action='legacy'){
    if(!active(n)||blocked()||!['legacy','punch','kick','grab'].includes(action))return false;
