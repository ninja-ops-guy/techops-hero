...
(function(root){
  ...
  function sound(id){try{if(typeof root.sfx==='function')root.sfx(id);}catch(_){/* Audio cannot interrupt a hit or reward. */}}
  function simulationBlocked(){
    const s=typeof S!=='undefined'?S:root.S,rt=root.TechOpsNightRuntime;
  ...
  const MOVES=Object.freeze({
    ...
    launcher:Object.freeze({damage:24,reach:80,windup:70,recovery:230,launch:true,vx:2,vy:-10.5}),
    uppercut:Object.freeze({damage:20,reach:82,windup:80,recovery:270,launch:true,vx:1.2,vy:-10.8}),
  ...
  function throwHeld(n,dir){
    ...
    lift(n,e,dir==='up'?sign*1.3:dir==='down'?0:sign*9.2,dir==='up'?-12:dir==='down'?3:-5.5,true);
    if(dir==='down')enemy(e).locked=true;
    damage(n,e,dir==='up'?16:dir==='down'?22:24,'throw');
    c.attack={kind:dir==='up'?'throw-up':'throw',at:c.time,hit:true,connected:true,launch:dir==='up',stage:0,duration:270,face:sign};
  ...
  function attack(n,k={},action='legacy'){
    if(!active(n)||simulationBlocked()||!['legacy','punch','kick','grab'].includes(action))return false;
  ...
  function jumpCancel(n){
    if(!active(n)||simulationBlocked())return false;const c=state(n),a=c.attack;
  ...
  function strike(n,a){
    ...
    a.connected=damage(n,e,value,a.kind)&&!shield;if(!e.alive)return;
  ...
  function tick(n,dt,k){if(!active(n))return {locked:false};if(n.drive||simulationBlocked())return {locked:true};const c=state(n);k=readInput(k);c.time+=(Number.isFinite(dt)?Math.min(Math.max(dt,0),.05):0)*1000;
  ...
  function draw(ctx,n){if(!active(n))return;const c=state(n),W=ctx.canvas.width,calm=root.matchMedia&&root.matchMedia('(prefers-reduced-motion: reduce)').matches;ctx.save();ctx.font='bold 11px monospace';ctx.textAlign='center';
    ...
    const width=Math.min(290,W-20),x=(W-width)/2,y=91;ctx.fillStyle='#08111de8';ctx.fillRect(x,y,width,42);ctx.fillStyle='#a9c4d9';ctx.fillText(c.grab?'← / → THROW   ↑ LAUNCH   ↓ SLAM':c.stunUntil>c.time?'RECOVER':c.attack?.kind?.startsWith('air')?'AIR COMBO · THREE HITS MAX':'↑ HIGH · ↓ LOW · KICK / GRAB',W/2,y+14);
  ...
  root.TechOpsNightCombat={VERSION:2,RULES,MOVES,active,state,attack,tick,stepEnemy,hurt,cancel,pose,draw,target,normalizeInput,readInput,jumpCancel,grab:(n,k)=>attack(n,k,'grab'),kick:(n,k)=>attack(n,k,'kick')};
