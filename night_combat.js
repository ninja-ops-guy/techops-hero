...
(function(root){
  ...
  function sound(id){try{if(typeof root.sfx==='function')root.sfx(id);}catch(e){/* Sound must never interrupt contact resolution. */}}
  ...
  function damage(n,e,value,kind,detail){if(!e.alive||e.hp<=0)return false;const c=state(n),ec=enemy(e);e.hp=Math.max(0,e.hp-Math.round(value));e.hitT=8;e.windup=0;ec.stunUntil=Math.max(ec.stunUntil,c.time+RULES.stun);n.hitStop=Math.max(n.hitStop||0,kind==='throw'||kind==='launcher'?5:3);event(n,kind,e,Object.assign({damage:Math.round(value)},detail||{}));if(root.TechOpsArtHandoff)root.TechOpsArtHandoff.impact(n,e,kind==='jab'?'hit':'finisher',wallClock());if(e.hp<=0){e.alive=false;ec.held=false;n.kills=(n.kills||0)+1;const cash=e.cash||[0,0],pay=cash[0]+Math.floor(Math.random()*(cash[1]-cash[0]+1));n.cash=(n.cash||0)+pay;event(n,'ko',e,{cash:pay});say(n,'KNOCKOUT · +$'+pay);if(typeof root.nmCheckClear==='function')root.nmCheckClear();}return true;}
  ...
  function attack(n,k){if(!active(n)||root.TechOpsNightSession&&root.TechOpsNightSession.paused(root.S))return false;const c=state(n);if(n.drive||n.block||n.hp<=0||c.stunUntil>c.time||root.S&&(root.S.inDialog||root.S.inBattle))return false;
  ...
  function strike(n,a){const c=state(n),e=target(n,{},false);if(!e){clearChain(n);event(n,'whiff');return;}const ec=enemy(e);if(ec.air&&ec.locked){clearChain(n);event(n,'whiff');return;}
    ...
    damage(n,e,value,a.kind,{beat:!!a.beat&&!shield,guarded:!!shield});if(!e.alive)return;
