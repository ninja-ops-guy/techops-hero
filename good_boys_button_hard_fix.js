/* TechOps Hero — 118/1984 hard title-button owner v7.
 * Physical-device title entry always starts the authored side-story opening:
 * cockpit/deck INTERACT -> GD_CUT_02 takeover -> playable space flight ->
 * GD_CUT_03 orbital/prison approach -> shuttle crash -> fresh M3 prison gameplay.
 *
 * M2 NIGHT LAUNCH is intentionally bypassed here: the authored cockpit/takeover/
 * flight sequence now owns launch. The title button also does not resume a
 * persisted mission while this campaign is under acceptance testing.
 */
(function(root){
  "use strict";
  ...
  var VERSION=7,lastLaunch=0,launching=false,depTimer=0;
  ...
  function phase(name,extra){root.__goodBoysOpeningPhase=Object.assign({phase:name,owner:"hard-title-button-v7",at:Date.now()},extra||{});}
  ...
  function waitForDeps(timeout){return new Promise(function(resolve,reject){var start=Date.now();function poll(){if(depsReady()){resolve(true);return;}if(Date.now()-start>=timeout){reject(new Error("Good Dogs opening v7 dependencies did not become ready"));return;}depTimer=root.setTimeout(poll,50);}poll();});}
  ...
  async function opening(source){phase("opening-dependencies");setButton("LOADING GOOD DOGS OPENING…",true);await waitForDeps(7000);var cine=root.GoodDogsCutscenes,o=root.TechOpsGoodBoysOpeningV4;
    /* GD_CUT_01 belongs to the earlier Earth-side Garage Receiver beat. */
    phase("cockpit-interact");setButton("COCKPIT INTERACTION…",true);var deck=await o.showDeckInteraction();if(!deck||deck.completed!==true)throw new Error("Cockpit interaction did not complete");root.__goodBoysHardDeckResult=deck;
    phase("takeover-cutscene");setButton("TAKING CONTROL OF SHIP…",true);var takeover=validMovieResult(await cine.play("GD_CUT_02",{force:true,muted:true}),"GD_CUT_02");root.__goodBoysHardTakeoverClip=takeover;
