 ...
 * but it never deletes or substitutes an authored Story Bible encounter.
 */
(function(root){
  "use strict";
  ...
  var ROUTE={
    ...
    5:{name:"THE MIKE INDEX / ACCESS CORE",objective:"BREAK THE MIKE INDEX · ESCORT K · SEIZE THE ROUTE CONTROLS · OPEN THE PATH TO 1984",zone:"BLACKSITE MERIDIAN — ORPHEUS ACCESS CORE",target:1480,color:"#d9e2ea"},
  ...
  var STAGES={
    ...
    5:{platforms:[[250,345,240],[590,345,240],[930,310,230],[1260,345,260]],hazards:[[845,45]],landmarks:[{x:760,label:"K — ROUTE KEY",kind:"k"},{x:980,label:"THE MIKE INDEX",kind:"boss"},{x:1070,label:"ACCESS NODE",kind:"console"},{x:1480,label:"ROUTE 1984",kind:"door"}]},
  ...
  function tick(){
    try{syncDefinitions();installDrawAuthority();if(!active()){lastMission=0;normalizedMission=0;return;}var m=mission();applyStage();if(m!==lastMission)lastMission=m;normalizeLegacy();if(m===1)updateWaldoTrail();var a=root.TechOpsGoodBoysBackgroundAuthority;if(a&&a.enforce)a.enforce();}catch(e){root.__goodBoysBibleWorldError=String(e&&e.stack||e);}
  }
  function acceptance(){var n=root.NM||{},m=mission(),r=ROUTE[m]||{};return{version:VERSION,active:active(),mission:m,name:r.name||null,objective:r.objective||null,stage:!!(n._gbBibleStageRevision===VERSION&&n._gbBibleStageMission===m),normalized:!!n._gbBibleMissionNormalized,waldoProperty:m!==1||!!n._gbWaldoPropertyCanonical,trailStep:Number(n._gbWaldoTrailStep)||0,trailComplete:!!n._gbWaldoTrailComplete,m5Canonical:m!==5||!!n._gbBibleM5Canonical,repairs:repairs,pass:!active()||!!(n._gbBibleStageRevision===VERSION&&n._gbBibleStageMission===m&&(m!==1||n._gbWaldoPropertyCanonical)&&(m!==5||n._gbBibleM5Canonical))};}
