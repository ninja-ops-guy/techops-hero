...
(function(root){
  ...
  function drawHud(x,NM){
    try{
      var c=cs();if(!c)return;canon();var W=x.canvas.width,m=mission(),s=seq(),cfg=s&&s[m]||{},small=W<=560,pad=small?6:10,gap=small?4:8,centerW=small?Math.min(112,Math.max(92,W*.26)):Math.min(190,W*.30),edgeW=Math.min(small?148:190,(W-centerW-pad*2-gap*2)/2),h=small?50:56,y=7;
  ...
  function acceptance(){var w=n();return{version:VERSION,installed:installed,active:active(),mission:mission(),authority:w&&w._goodBoysHudAuthority||null,kAt118:!!(seq()&&seq()[4]&&/K/.test(seq()[4].objective||"")),waldoAt1984:!!(seq()&&seq()[6]&&/WALDO/.test(seq()[6].objective||"")),drawError:root.__goodBoysHudLiteDrawError||null,installError:root.__goodBoysHudLiteInstallError||null};}
