/* TechOps Hero — Good Boys single-HUD authority v3.
...
(function(root){
  ...
  function drawHud(x,NM){
    try{
      ...
      var W=x.canvas.width,m=mission(),s=seq(),cfg=s&&s[m]||{},guide=root.TechOpsGameplayExperience&&root.TechOpsGameplayExperience.objective(NM),small=W<=560,pad=small?5:8,gap=small?3:6,centerW=small?Math.min(96,Math.max(82,W*.23)):Math.min(160,W*.27),edgeW=Math.min(small?132:176,(W-centerW-pad*2-gap*2)/2),h=small?38:46,y=4;
      function card(who,left){
        ...
        x.save();x.fillStyle="rgba(2,8,14,.88)";x.strokeStyle=col;x.lineWidth=1.5;box(x,px,y,edgeW,h,6);x.fill();x.stroke();x.textAlign="left";x.fillStyle=col;x.font="bold "+(small?9:10)+"px monospace";x.fillText(who.toUpperCase(),px+6,y+11);bar(x,px+6,y+16,edgeW-12,7,hp,mx,C.red);x.fillStyle="#d9edfb";x.font="bold 7px monospace";x.fillText(Math.max(0,Math.round(hp))+" / "+mx,px+6,y+h-5);if(who===aw){x.textAlign="right";x.fillStyle=col;x.fillText(root.TechOpsGoodDogsCoop&&root.TechOpsGoodDogsCoop.active()?"P1":"ACTIVE",px+edgeW-6,y+h-5);}x.restore();
      }
      x.save();x.fillStyle="#071019";x.fillRect(0,0,W,small?80:86);x.restore();
      ...
      x.save();x.fillStyle="rgba(2,8,14,.90)";x.strokeStyle=accent;x.lineWidth=1.25;box(x,cx,y,centerW,h,6);x.fill();x.stroke();x.textAlign="center";x.fillStyle=accent;x.font="bold "+(small?8:9)+"px monospace";x.fillText(fit(x,cfg.name||("M"+m),centerW-8),W/2,y+11);x.fillStyle="#d9edfb";x.font="bold 7px monospace";x.fillText("SYNC "+Math.round(sync)+"%",W/2,y+22);bar(x,cx+7,y+28,centerW-14,6,sync,100,sync>=100?C.green:C.cyan);x.restore();
      var oy=y+h+4,ow=W-pad*2;x.save();x.fillStyle="rgba(2,8,14,.80)";x.strokeStyle="rgba(120,210,255,.24)";x.lineWidth=1;box(x,pad,oy,ow,28,4);x.fill();x.stroke();x.textAlign="center";x.fillStyle="#eef8ff";x.font="bold "+(small?8:9)+"px monospace";x.fillText(fit(x,guide?guide.text:cfg.objective||"",ow-16),W/2,oy+11);if(guide&&guide.detail){x.fillStyle="#b8dbea";x.font=(small?7:8)+"px monospace";x.fillText(fit(x,guide.detail,ow-16),W/2,oy+23);}x.restore();
