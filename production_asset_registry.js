/* TechOps Hero — complete production asset registry v1.
 * Makes every shipped visual asset discoverable by runtime, preloads physical
 * PNG assets, and loads previously-unwired atlas/reference manifests.
 */
(function(root){
  "use strict";
  ...
  var images={},loadedScripts={},failedScripts={},json={};
  ...
  function loadScript(src){return new Promise(function(resolve){try{
    if(scriptAlready(src)){loadedScripts[src]=true;resolve(true);return;}
    var s=root.document.createElement("script");s.src=src;s.async=false;s.dataset.productionAsset=src;
    s.onload=function(){loadedScripts[src]=true;resolve(true);};s.onerror=function(){failedScripts[src]=true;resolve(false);};
    (root.document.head||root.document.documentElement).appendChild(s);
  }catch(e){failedScripts[src]=true;resolve(false);}});}
  function preloadImage(src){return new Promise(function(resolve){try{var im=new Image();images[src]=im;im.onload=function(){resolve(true);};im.onerror=function(){resolve(false);};im.src=src;}catch(e){resolve(false);}});}
  function loadJSON(src){return root.fetch?root.fetch(src).then(function(r){if(!r.ok)throw 0;return r.json();}).then(function(v){json[src]=v;return true;}).catch(function(){return false;}):Promise.resolve(false);}
  async function install(){
    ...
    root.__productionAssetImages=images;root.__productionAssetJSON=json;
    ...
    root.__productionAssetFailures=Object.keys(failedScripts);
    root.__allProductionAssetsIntegrated=Object.keys(failedScripts).length===0;
    return root.__allProductionAssetsIntegrated;
  }
  function status(){return{version:VERSION,integrated:!!root.__allProductionAssetsIntegrated,scripts:SCRIPT_ASSETS.length,sourceParts:SOURCE_PARTS.length,png:PNG_ASSETS.length,json:JSON_ASSETS.length,failures:Object.keys(failedScripts)}};
