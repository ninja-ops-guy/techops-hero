...
(function(root){
  ...
  function visible(el){try{if(!el||el.classList.contains("hidden"))return false;var s=root.getComputedStyle?root.getComputedStyle(el):el.style;return !s||(s.display!=="none"&&s.visibility!=="hidden");}catch(e){return false;}}
  function hasBlockingModal(){try{var director=root.TechOpsPresentationDirector;if(director&&typeof director.isBlocking==="function"&&director.isBlocking())return true;var d=root.document;if(!d)return false;var ids=["dialogue","battle","eod","good-boys-campaign-intro","good-boys-mobile-recovery","good-boys-story-cine","good-boys-ship-interlude","good-boys-ship-flight","good-boys-crash-canonical","good-dogs-cutscene-overlay","gb-prison-cine","good-boys-earthfall-cine"];for(var i=0;i<ids.length;i++)if(visible(d.getElementById(ids[i])))return true;return false;}catch(e){return false;}}
