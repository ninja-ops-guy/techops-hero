...
assert.ok(source.includes("20260903-good-ship-atlas-r1"),"ship approach must bypass the corrupt mobile-cached atlas");
assert.ok(flight.includes("VERSION=4"),"canonical ship flight must require the repaired atlas loader");
assert.ok(flight.includes("20260903-good-ship-atlas-r1"),"canonical ship flight must bypass the corrupt mobile-cached atlas");
assert.deepStrictEqual(pngInfo("assets/good_boys/good_ship_arcade.atlas.png"),{width:768,height:520});
