...
// world drawing and unique center messages.
{
  const calls=[];
...
// Suppression is tied to exact known duplication and the visible HUD read
// model. Similar dialogue, combat feedback and mission results survive.
for(const width of [320,640])for(const message of ["VERIFY THE PRISONER IN CELL 118","VERIFY THE PRISONER IN CELL 118 — NEW CLUE","PERFECT TIMING","K: I REMEMBER THIS PLACE","CELL 118 SECURED — +40"]){
  const calls=[],font=width<620?10:13,mw=Math.min(520,width-16),ctx={canvas:{width,height:720},measureText:s=>({width:String(s).length*4}),fillRect(...a){calls.push(["rect",...a]);},strokeRect(...a){calls.push(["stroke",...a]);},fillText(...a){calls.push(["text",...a]);},drawImage(){}};
  const context={console,ctx,NM:{_v736:{m:4},msg:message},__goodBoysHudLiteInstalled:true,TechOpsGameplayExperience:{objective:()=>({mission:4,text:"VERIFY THE PRISONER",phase:"investigate"})}};
  context.__techopsFinalParserDrawNM=()=>{ctx.fillRect((width-mw)/2,96,mw,12+font+4);ctx.fillText(message,width/2,106+font+3);ctx.fillText("WORLD LABEL",width/2,220);};context.globalThis=context;vm.createContext(context);vm.runInContext(fs.readFileSync("good_boys_legacy_hud_filter.js","utf8"),context);
  const before=JSON.stringify(context.NM);context.__techopsFinalParserDrawNM();const duplicate=message==="VERIFY THE PRISONER IN CELL 118";
  assert.strictEqual(calls.some(c=>c[0]==="text"&&c[1]===message),!duplicate,"only the exact repeated briefing loses its center text");
  assert.strictEqual(calls.some(c=>c[0]==="rect"),!duplicate,"duplicate strip cannot leave an empty rectangle");assert.ok(calls.some(c=>c[1]==="WORLD LABEL"));assert.strictEqual(JSON.stringify(context.NM),before,"presentation filtering cannot mutate messages or mission state");
...
{
  const calls=[],ctx={canvas:{width:640,height:720},fillRect(...a){calls.push(["rect",...a]);},strokeRect(...a){calls.push(["stroke",...a]);},fillText(...a){calls.push(["text",...a]);},drawImage(){}},context={console,ctx,NM:{cam:1000,_v736:{m:4,evidence:[]}},__goodBoysHudLiteInstalled:true,TechOpsOrbitalStaging:{profile:()=>({id:"detention"})},TechOpsArtHandoff:{image:()=>({})},TechOpsLevelRegistry:{goodDogsCell118:()=>({entranceX:1600,labelX:1645})}};
  context.__techopsFinalParserDrawNM=()=>{ctx.strokeRect(600,270,90,160);ctx.fillText("CELL 118",645,258);ctx.strokeRect(400,310,180,10);};context.globalThis=context;vm.createContext(context);vm.runInContext(fs.readFileSync("good_boys_legacy_hud_filter.js","utf8"),context);context.__techopsFinalParserDrawNM();assert.deepStrictEqual(calls,[["stroke",400,310,180,10]],"decoded approved door replaces only the aligned placeholder, preserving platforms");calls.length=0;context.TechOpsArtHandoff.image=()=>null;context.__techopsFinalParserDrawNM();assert.strictEqual(calls.length,3,"the real interaction marker survives until approved source art is decoded");
}
...
assert.ok(hard.includes("VERSION=16"),"hard title button must be the M1-first v16 opening authority");assert.ok(hard.includes("function freshConfig(){return{mission:1"),"fresh Good Dogs must mount playable M1");assert.ok(!hard.includes('GoodDogsCutscenes.play("GD_CUT_01"'),"title authority must not play ship footage before M1");assert.ok(!hard.includes("TechOpsGoodBoysShipFlight"),"title authority may not bypass M1/M2 by launching the ship sequence");assert.ok(!hard.includes('GoodDogsCutscenes.play("GD_CUT_02"'),"M2, not the title button, owns the takeover film");
assert.ok(deck.includes("assets/v736/good_boys_ship/cockpit_pilot.jpg"),"cockpit must integrate the existing pilot asset");assert.ok(deck.includes("pilotX=480"),"cockpit interaction target must sit on the center pilot");assert.ok(deck.includes("MOVE TO PILOT")&&deck.includes("INTERACT"),"pilot must be a gameplay interaction target");assert.ok(deck.includes("VERSION=7"),"deck scene must retain the Good Ship gameplay asset integration contract");assert.ok(flight.includes("assets/good_boys/good_ship_arcade.atlas.png"),"flight must use the supplied Good Ship atlas");assert.ok(flight.includes("VERSION=6"),"ship authority must expose the M2 exit-sequence v6 contract");assert.ok(flight.includes("20260903-good-ship-gameplay-assets-r2"),"flight must bypass the stale strip atlas");assert.ok(flight.includes("asteroid_1")&&flight.includes("ship_player")&&flight.includes("prison_station"),"flight must use supplied ship/hazard/prison frames");assert.ok(flight.includes("runBoardingSequence")&&flight.includes("_gbBoardSequenceComplete"),"M2 must own the complete deck-to-crash handoff");
assert.ok(cutscenes.includes('VERSION:"3.7"'),"cutscene player must expose the capability-gated v3.7 playback contract");assert.ok(cutscenes.includes("freshVideoNode:true"),"each cinematic session must own a fresh video element");assert.ok(/cleanup\(\);\s*try\{video\.pause\(\)/.test(cutscenes),"cutscene teardown must detach callbacks before touching media");assert.ok(!cutscenes.includes('video.removeAttribute("src");video.load()'),"cutscene finish may not synchronously reload an aborted media element");
assert.ok(cutscenes.includes("02_signal_pull_transition_pixel.mp4?v=20260903-picked-pilot-attack-r1"),"takeover cutscene must cache-bust the picked cockpit attack clip");assert.ok(fs.existsSync("assets/cutscenes/good_dogs/02_signal_pull_transition_pixel.mp4"),"picked cockpit attack clip must be present");assert.ok(cutscenes.includes("attemptAutoplay"),"cutscene player must attempt muted inline autoplay");assert.ok(cutscenes.includes("video.autoplay=autoRequested")&&cutscenes.includes('video.setAttribute("autoplay","")'),"autoplay must be declared on the fresh video node");assert.ok(/video\.src=options\.src\|\|def\.src;try\{video\.load\(\);\}catch\(_\)\{\}\s*if\(autoRequested\)attemptAutoplay\(\)/.test(cutscenes),"autoplay must be attempted immediately after source/load instead of waiting on metadata events");assert.ok(cutscenes.includes("immediate:true"),"autoplay diagnostics must record the immediate-attempt contract");assert.ok(!/STARTING|TRANSMISSION/.test(cutscenes),"cutscene player must not flash non-bible transmission text over the film");assert.ok(cutscenes.includes(".gd-playing .gd-film-status{opacity:0}"),"cutscene recovery text must stay hidden while authored footage plays");assert.ok(!cutscenes.includes('if(ios)waitForUser("ios-ready"'),"iPhone must not be forced through a manual VIDEO READY gate");assert.ok(flight.includes('c.play("GD_CUT_02",{force:true,muted:true,autoplay:true,noPoster:true})'),"M2 takeover clip must request autoplay without a poster plate");assert.ok(!flight.includes('play("GD_CUT_03"'),"the extra flying-ship cutscene must remain retired");
