...
assert.ok(hard.includes("VERSION=11"),"hard title button must be the v11 opening authority");assert.ok(hard.includes("TechOpsGoodBoysShipFlight"),"opening must call the canonical ship-flight authority directly");assert.ok(!hard.includes("o.showSpaceFlight()"),"opening may not route through the legacy flight alias");assert.ok(hard.includes("result.completed!==true"),"opening must reject incomplete canonical flight callbacks");assert.ok(hard.includes("result&&result.assetError"),"canonical flight asset failures must fail closed before the prison cutscene");
assert.ok(deck.includes("assets/v736/good_boys_ship/cockpit_pilot.jpg"),"cockpit must integrate the existing pilot asset");assert.ok(deck.includes("pilotX=480"),"cockpit interaction target must sit on the center pilot");assert.ok(deck.includes("MOVE TO PILOT")&&deck.includes("INTERACT"),"pilot must be a gameplay interaction target");assert.ok(flight.includes("assets/good_boys/good_ship_arcade.atlas.png"),"flight must use the supplied Good Ship atlas");assert.ok(flight.includes("VERSION=4"),"flight must require the repaired atlas loader");assert.ok(flight.includes("asteroid_1")&&flight.includes("ship_player")&&flight.includes("prison_station"),"flight must use supplied ship/hazard/prison frames");assert.ok(cutscenes.includes('VERSION:"3.2"'),"cutscene player must expose automatic iPhone playback contract");assert.ok(cutscenes.includes("attemptAutoplay"),"cutscene player must attempt muted inline autoplay");assert.ok(!cutscenes.includes("STARTING TRANSMISSION"),"cutscene player must not flash the non-bible transmission title over the film");assert.ok(!cutscenes.includes('if(ios)waitForUser("ios-ready"'),"iPhone must not be forced through a manual VIDEO READY gate");assert.ok(hard.includes('cine.play("GD_CUT_02",{force:true,muted:true,autoplay:true})'),"takeover clip must request autoplay");assert.ok(hard.includes('cine.play("GD_CUT_03",{force:true,muted:true,autoplay:true})'),"prison approach clip must request autoplay");
