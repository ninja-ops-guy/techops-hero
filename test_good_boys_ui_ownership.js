...
assert.ok(hard.includes("VERSION=14"),"hard title button must be the M1-first v14 opening authority");assert.ok(hard.includes("function freshConfig(){return{mission:1"),"fresh Good Dogs must mount playable M1");assert.ok(hard.includes('GoodDogsCutscenes.play("GD_CUT_01"'),"title authority must own only the opening signal film");assert.ok(!hard.includes("TechOpsGoodBoysShipFlight"),"title authority may not bypass M1/M2 by launching the ship sequence");assert.ok(!hard.includes('GoodDogsCutscenes.play("GD_CUT_02"'),"M2, not the title button, owns the takeover film");
