...
(function(root){
  ...
  function buildBackdrops(){
    ...
    try{root.NM_BG734=root.NM_BG734||{};var authored=root.TechOpsM3CinematicAsset&&root.TechOpsM3CinematicAsset.image&&root.TechOpsM3CinematicAsset.image();root.NM_BG734.goodboys_breach=authored||root.NM_BG734.goodboys_breach||makeBackdrop('breach');root.NM_BG734.goodboys_cell118=makeBackdrop('cell118');root.NM_BG734.goodboys_core=makeBackdrop('core');root.NM_BG734.goodboys_cell1984=makeBackdrop('cell1984');root.NM_BG734.goodboys_escape=makeBackdrop('escape');built=true;root.__goodBoysPrisonBackdropsReady=true;root.__goodBoysM3VisualAuthority=authored?"authored-mixed-media":"generated-fallback-pending";return true;}catch(e){root.__goodBoysPrisonBackdropError=String(e&&e.stack||e);return false;}finally{building=false;}
  }
  ...
  function bgForMission(m){try{if(m===3){var authored=root.TechOpsM3CinematicAsset&&root.TechOpsM3CinematicAsset.image&&root.TechOpsM3CinematicAsset.image();if(authored&&authored.src)return authored.src;}var p=root.GOOD_BOYS_CUTSCENE_PLATES,pk=m===3?'orbital_approach':m===4?'cell_118':m===5?'access_core':m===6?'cell_1984':'warden_shuttle_bay';if(p&&p[pk])return p[pk];buildBackdrops();var k=m===3?'goodboys_breach':m===4?'goodboys_cell118':m===5?'goodboys_core':m===6?'goodboys_cell1984':'goodboys_escape',im=root.NM_BG734&&root.NM_BG734[k];return im&&im.src?im.src:'';}catch(e){return '';}}
  ...
  function showPrisonCinematic(m,done){
    ...
    function render(){var s=scenes[i],src=bgForMission(m);o.style.backgroundImage=src?'linear-gradient(to bottom,rgba(0,0,0,.08),rgba(0,0,0,.55)),url("'+String(src).replace(/"/g,'%22')+'")':'linear-gradient(#07111b,#02050a)';o.innerHTML='<div class="card"><div class="eyebrow">GOOD DOGS PROTOCOL · ORBITAL DETENTION</div><h2>'+s.t+'</h2><p>'+s.p+'</p><div class="goal">'+s.g+'</div><button id="gb-prison-next">'+(i===scenes.length-1?'TAKE CONTROL':'CONTINUE')+'</button></div>';o.querySelector('#gb-prison-next').addEventListener('pointerdown',next,{passive:false});}
