/* Good Boys deterministic progression authority v2.
...
(function(root){
  ...
  var VERSION=2,timer=null,observer=null,missionSeen=0,seenEnemy=false,emptySince=0,transition=false,lastAdvance=0;
  ...
  function cinematicVisible(){try{if(!root.document)return false;var a=root.document.getElementById("good-boys-story-cine"),b=root.document.getElementById("gb-prison-cine");function visible(el){if(!el)return false;var s=root.getComputedStyle?root.getComputedStyle(el):el.style;return !s||s.display!=="none"&&s.visibility!=="hidden"&&Number(s.opacity||1)!==0;}return visible(a)||visible(b);}catch(e){return false;}}
  ...
  function tick(){
    try{
      ...
      /* Mission 1 is traversal, not combat. Never let stray/carry-over enemies
         suppress reaching the hidden bay. This was the M1->M2 CI race. */
      if(m===1){if(Number(n.x)>=1325)advance(2,"reached-hidden-bay");return;}
      var alive=living();
      if(m===2){
        if(alive>0){seenEnemy=true;emptySince=0;return;}
        if(seenEnemy&&!pending())revealShip();
      ...
      if(m>=3&&m<=7){
        ...
        if(now()-emptySince>=900)advance(m+1,"encounter-clear");
      }
