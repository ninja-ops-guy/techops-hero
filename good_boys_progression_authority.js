/* Good Dogs deterministic progression authority v14.
...
(function(root){
  ...
  var VERSION=14,timer=null,observer=null,missionSeen=0,seenEnemy=false,emptySince=0,transition=false,lastAdvance=0,repairs=0;
  ...
  function cinematicVisible(){try{var d=root.TechOpsPresentationDirector;if(d&&typeof d.isBlocking==="function"&&d.isBlocking("gooddogs"))return true;if(!root.document)return false;var ids=["good-boys-story-cine","gb-prison-cine","good-boys-campaign-intro","good-boys-earthfall-cine","good-boys-ship-interlude","good-boys-ship-flight","good-boys-crash-canonical","good-dogs-cutscene-overlay"];for(var i=0;i<ids.length;i++){var el=root.document.getElementById(ids[i]);if(!el)continue;var s=root.getComputedStyle?root.getComputedStyle(el):el.style;if(!s||s.display!=="none"&&s.visibility!=="hidden"&&Number(s.opacity||1)!==0)return true;}return false;}catch(e){return false;}}
  ...
  function resetWorldForHandoff(){try{var n=root.NM,c=cs();if(c){c.ending=true;c.pendingSpawn=null;}if(n){n.enemies=[];n.clear=false;n._gbShipRevealed=false;n.x=180;n.cam=0;n.hitStop=0;}var d=root.TechOpsCameraDirector;if(d&&d.reset)d.reset("night-world");}catch(_){} }

  function completionStatus(which){
    try{
      var c=cs(),n=root.NM,m=meta(),registry=root.TechOpsLevelRegistry,row=registry&&registry.goodDogsMission?registry.goodDogsMission(which):null,contract=row&&row.completionContract||"legacy";
      if(!c||!n)return{ok:false,mission:which,contract:contract,reason:"runtime-unavailable"};
      var ok=false,reason="objective-incomplete";
      if(which===1){var pair=!!(c.chars&&c.chars.katrin&&c.chars.manchez&&c.partner&&Math.abs(Number(c.partner.x||0)-Number(n.x||0))<180);ok=!!(n._gbWaldoTrailComplete&&n._gbHiddenBayEntered&&pair);reason=!n._gbWaldoTrailComplete?"trail-incomplete":!n._gbHiddenBayEntered?"hidden-bay-not-entered":!pair?"pair-not-assembled":"complete";}
      else if(which===2){ok=!!(n._gbBoardSequenceComplete&&c._gbBoardSequenceComplete);reason=ok?"complete":"boarding-sequence-incomplete";}
      else if(which===3){ok=!!(c.prisonOpsV2&&c.prisonOpsV2.complete);reason=ok?"complete":"breach-ops-incomplete";}
      else if(which===4){var clues=(c.evidence||[]).filter(function(e){return e&&e.found;}).length;ok=!!(clues>=3&&c.cellOpened&&c._gbCell118AmbushCommitted&&c._gbCell118AmbushCleared);reason=ok?"complete":"cell118-rescue-incomplete";}
      else if(which===5){ok=!!(c._gbMikeIndexDefeated&&c._gbAccessNodeSeized);reason=ok?"complete":!c._gbMikeIndexDefeated?"mike-index-not-defeated":"access-node-not-seized";}
