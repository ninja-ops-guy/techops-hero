/* Good Boys deterministic progression authority v9.
 * Canonical campaign state lives in S.meta._v736. Runtime NM._v736 is a mirror.
 * All mission transitions are validated, persisted, mirrored, and checked before
 * the next mission can consume gameplay state.
 */
(function(root){
  "use strict";
  if(!root)return;
  ...
  var VERSION=9,timer=null,observer=null,missionSeen=0,seenEnemy=false,emptySince=0,transition=false,lastAdvance=0,repairs=0;
  ...
  function hardenStart(){
    try{
      ...
      var wrapped=function(options){
        ...
        var r=base.call(this,options),c=cs(),expected=m?clampMission(m.m):clampMission(options.mission||1);
        if(c&&Number(c.m)!==expected)c.m=expected;
        reconcileMission("v736-start-wrapper");
        invariant(expected,"v736-start-wrapper");
        return r;
      };
  ...
  function startNext(next){
    try{var m=meta();if(!m)return false;next=clampMission(next);hardenStart();if(root.S&&!cinematicVisible())root.S.inDialog=false;
      ...
      if(root.v736&&typeof root.v736.start==="function"){
        ...
        var c2=cs(),observed=Number(c2&&c2.m||0);if(c2&&observed!==next)c2.m=next;if(observed!==next)root.__goodBoysForcedRepair={expected:next,observed:observed,at:Date.now(),source:"v736.start"};
        reconcileMission("after-v736-start");if(!invariant(next,"after-v736-start"))throw new Error("Good Boys mission invariant failed after v736.start");
        var validator=root.TechOpsStateValidator;if(validator&&typeof validator.validateCampaign==="function"){var vr=validator.validateCampaign(m,c2);if(!vr.valid){root.__stateValidationFailure=vr;throw new Error("Good Boys state validator failed: "+vr.errors.join(" | "));}}
