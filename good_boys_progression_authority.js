...
(function(root){
  ...
  function reconcileMission(reason){
    try{
      var c=cs(),m=meta();if(!c||!m)return false;var canonical=clampMission(m.m),runtime=clampMission(c.m||canonical);
      if(runtime!==canonical||Number(c.m)!==canonical){
        c.m=canonical;repairs++;
        root.__goodBoysMissionRepair={runtime:runtime,canonical:canonical,reason:reason||"reconcile",at:Date.now(),count:repairs};
      }
      return true;
    }catch(e){root.__goodBoysMissionRepairError=String(e&&e.stack||e);return false;}
  }
  ...
  function cinematicVisible(){try{if(!root.document)return false;var a=root.document.getElementById("good-boys-story-cine"),b=root.document.getElementById("gb-prison-cine"),c=root.document.getElementById("good-boys-campaign-intro"),d=root.document.getElementById("good-boys-earthfall-cine");function visible(el){if(!el)return false;var s=root.getComputedStyle?root.getComputedStyle(el):el.style;return !s||s.display!=="none"&&s.visibility!=="hidden"&&Number(s.opacity||1)!==0;}return visible(a)||visible(b)||visible(c)||visible(d);}catch(e){return false;}}
  ...
  function startNext(next){
    try{
      ...
      if(root.v736&&typeof root.v736.start==="function"){
        root.v736.start();
        reconcileMission("after-v736-start");
        if(root.setTimeout)root.setTimeout(function(){reconcileMission("post-start-settle");},80);
        return true;
      }
  ...
  root.TechOpsGoodBoysProgressionAuthority={VERSION:VERSION,tick:tick,advance:advance,revealShip:revealShip,startNext:startNext,mission:mission,reconcileMission:reconcileMission,acceptance:acceptance,testPrimeClear:testPrimeClear,cinematicVisible:cinematicVisible,enforceCinematicBlock:enforceCinematicBlock,removeCompetingPads:removeCompetingPads,observer:observer,timer:timer};
  tick();
