...
(function(root){
  ...
  function visible(id){try{var o=root.document&&root.document.getElementById(id);if(!o||o.classList.contains("hidden"))return false;var s=root.getComputedStyle?root.getComputedStyle(o):o.style;return !s||(s.display!=="none"&&s.visibility!=="hidden"&&Number(s.opacity||1)!==0);}catch(e){return false;}}
  function introVisible(){try{return visible("good-boys-campaign-intro")||visible("good-boys-story-cine")||visible("gb-prison-cine")||visible("good-boys-earthfall-cine")||visible("v725-cine");}catch(e){return false;}}
  ...
  function closeShell(){try{if(root.document)["dialogue","panel","eod","battle"].forEach(function(id){var e=root.document.getElementById(id);if(e)e.classList.add("hidden");});var s=state();if(s&&!introVisible())s.inDialog=false;}catch(e){}}
  function primeNight(){try{if(introVisible())return false;closeShell();var f=enterNightFn();if(!nightReady()&&f)f();}catch(e){root.__goodBoysMobilePrimeError=String(e&&e.stack||e);}try{if(root.v722&&typeof root.v722.active==="function"&&root.v722.active()&&typeof root.v722.skip==="function")root.v722.skip();}catch(e){}return true;}

  /* Safe state repair only. The previous implementation called three feature
     tick() functions every 80ms; each tick contains "install if marker missing"
     wrapper logic, which could form gameplay -> canon -> gameplay cycles. */
