...
test('bootstrap retains latest main Night flow and mobile controls exactly once',()=>{const src=fs.readFileSync('production_bootstrap.js','utf8'),r={setTimeout(){}};vm.runInNewContext(src,r);for(const file of ['runtime_night.js','good_boys_reference_ui_v1.js','runtime_combat_audio.js','production_gameplay_experience.js','orbital_scene_staging.js'])assert.equal(r.TechOpsProductionBootstrap.FILES.filter(x=>x===file).length,1,file);assert.match(src,/TechOpsNightFlow\.install\(\)/);assert.match(src,/TechOpsGoodBoysReferenceUI\.apply\(\)/);});
