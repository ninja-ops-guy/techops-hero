...
try{
  ...
  await page.waitForFunction(()=>!!(window.NM&&window.NM._v736),null,{timeout:9000});await page.waitForTimeout(650);a=await snap(page);log('gameplay',a);assertOwner(a,'gameplay');if(!(a.clockIn&&a.clockIn.ok&&a.clockIn.difficulty==='standard'))fail('canonical-standard-clock-in-failed',a);if(!a.campaign||a.mission!==2||a.metaMission!==2||a.stateMission!==2)fail('m2-canonical-handoff-failed',a);if(a.invariant&&a.invariant.ok===false)fail('mission-invariant-failed',a);if(a.active||a.premise||a.interlude)fail('opening-overlay-remains',a);if(!(a.sState&&a.sState.GD_CUT_01&&a.sState.GD_CUT_02))fail('opening-state-not-migrated',a);await page.screenshot({path:path.join(OUT,'goodboys-gameplay-m2.png')});
