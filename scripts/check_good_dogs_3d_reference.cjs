...
(async()=>{
 ...
 for(const view of ['third','retro','first','crew']){
  ...
  for(const [key,direction] of [['KeyW',sign],['KeyS',-sign],['ArrowRight',sign],['ArrowLeft',-sign]]){
   const before=await page.evaluate(()=>NM.x);await page.keyboard.down(key);
   try{await page.waitForFunction(({before,direction})=>(NM.x-before)*direction>8,{before,direction},{timeout:5000});}finally{await page.keyboard.up(key);}
   controls.push({view,key,direction,pass:true});
  }
 ...
 for(const [hold,direction] of [['forward',1],['back',-1]]){
  const b=await mobile.locator('[data-hold='+hold+']').boundingBox(),before=await mobile.evaluate(()=>NM.x);
  await cdp.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{x:b.x+b.width/2,y:b.y+b.height/2}]});
  try{await mobile.waitForFunction(({before,direction})=>(NM.x-before)*direction>8,{before,direction},{timeout:5000});}finally{await cdp.send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});}
  controls.push({view:'third',touch:hold,direction,pass:true});
 }
