...
async function dismiss(page,ms=6500){const until=Date.now()+ms;while(Date.now()<until){const b=page.locator('#good-boys-story-cine button,#gb-prison-cine button,#good-boys-campaign-intro button');if(await b.count()){await b.first().click({timeout:500}).catch(()=>{});await page.waitForTimeout(100);continue;}const txt=await page.locator('body').innerText().catch(()=>'');if(/SELECT SHIFT DIFFICULTY/i.test(txt)){await clickText(page,/Standard/i);continue;}if(/BEGIN THE INCIDENT/i.test(txt)){await clickText(page,/BEGIN THE INCIDENT/i);continue;}break;}}
...
try{
  ...
  for(let m=1;m<=8;m++){
    await page.evaluate((mission)=>{var n=window.NM,s=window.S;if(n&&n._v736)n._v736.m=mission;if(s&&s.meta&&s.meta._v736)s.meta._v736.m=mission;var p=window.TechOpsGoodBoysPrisonCinematicPatch;if(p&&p.buildBackdrops)p.buildBackdrops();var a=window.TechOpsGoodBoysBackgroundAuthority;if(a){a.syncCanon();a.enforce();}var w=window.TechOpsGoodBoysBibleWorld;if(w)w.tick();},m);
    await page.waitForTimeout(m>=3&&m<=7?650:300);
    const st=await page.evaluate(()=>{var a=window.TechOpsGoodBoysBackgroundAuthority,w=window.TechOpsGoodBoysBibleWorld,n=window.NM||{},c=window.TechOpsGoodBoysCanon&&window.TechOpsGoodBoysCanon.canonical&&window.TechOpsGoodBoysCanon.canonical(),g=window.TechOpsGoodBoysGameplayLoop;return {authority:!!a,bibleWorld:!!w,acceptance:a&&a.acceptance?a.acceptance():null,worldAcceptance:w&&w.acceptance?w.acceptance():null,district:n.district||null,canonBackground:n._goodBoysCanonBackground||null,canonScene:n._goodBoysCanonScene||null,property:!!n._gbWaldoPropertyCanonical,landmarks:(n._goodBoysLandmarks||[]).map(x=>x.label),stageMission:Number(n._gbBibleStageMission||0),canonRow:c?{bg:c.bg,district:c.district,name:c.name,zone:c.zone,objective:c.objective}:null,phase:g&&g.PHASES?g.PHASES[(n._v736&&n._v736.m)||0]:null,abstractEye:n._goodBoysCanonBackground==='orbital_eye',abstractGate:n._goodBoysCanonBackground==='orbital_gate'};});
    const e=expected[m];
