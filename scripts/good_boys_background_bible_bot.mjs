...
try{
  ...
  await page.evaluate(()=>{window.__goodBoysBackgroundBibleSampling=true;const p=window.TechOpsGoodBoysProgressionAuthority;if(p&&p.timer!=null){clearInterval(p.timer);p.timer=null;}if(p&&typeof p.tick==='function'){p.__backgroundBibleLiveTick=p.tick;p.tick=function(){return false;};}const s=window.TechOpsGoodBoysCampaignState;if(s&&typeof s.transition==='function'&&!s.__backgroundBibleLiveTransition){s.__backgroundBibleLiveTransition=s.transition;s.transition=function(){if(window.__goodBoysBackgroundBibleSampling)throw new Error('background-bible-sampling-transition-blocked');return s.__backgroundBibleLiveTransition.apply(this,arguments);};}});
  for(let m=1;m<=8;m++){
