...
async function snapshotRuntime(page) {
  return page.evaluate(() => {
    ...
    let controls=null;
    try{
      const box=document.getElementById('good-dogs-touch'),buttons=box?Array.from(box.querySelectorAll('button')):[],rects=buttons.map(b=>{const r=b.getBoundingClientRect(),s=getComputedStyle(b);return{id:b.id,x:r.x,y:r.y,w:r.width,h:r.height,display:s.display,borderRadius:s.borderRadius,background:s.backgroundColor};});
      let overlaps=false;for(let i=0;i<rects.length;i++)for(let j=i+1;j<rects.length;j++){const a=rects[i],b=rects[j];if(a.w&&a.h&&b.w&&b.h&&a.x<b.x+b.w&&a.x+a.w>b.x&&a.y<b.y+b.h&&a.y+a.h>b.y)overlaps=true;}
      controls={bodyClass:document.body.classList.contains('good-boys-controls'),visible:vis('good-dogs-touch'),count:buttons.length,overlaps,rects};
    }catch(e){controls={error:String(e)};}
    ...
    const routerTrace=Array.isArray(window.__productionNightLaunchTrace)?window.__productionNightLaunchTrace.slice(-12):[];
    const sc=document.getElementById('sc-widget');
    return {url:location.href,title:document.title,buttons:Array.from(document.querySelectorAll('button')).filter(b=>getComputedStyle(b).display!=='none').map(b=>b.textContent.trim()).filter(Boolean),titleVisible:vis('title-screen'),hudVisible:vis('hud'),dialogueVisible:vis('dialogue'),touchVisible:vis('touch-ui'),legacyNightControlsVisible:vis('v55-nmbtns'),recoveryVisible:!!document.getElementById('good-boys-mobile-recovery'),toast:text('toast'),card:text('v63-card'),bodyText,runtime,canvas,controls,cutsceneVisible:legacyCutscene||!!blockerId,authoredBlocker:!!blockerId,blockerId,shipInteraction:window.__goodBoysOpeningGameplay||null,openingPhase:window.__goodBoysOpeningPhase||null,mode:window.__productionActiveMode||window.__productionDesiredMode||null,routerPhase:window.__productionNightLaunchPhase||null,routerTrace,routerError:window.__productionModeRouterError||null,safetyError:window.__techOpsLastRuntimeError||null,goodBoysError:window.__goodBoysCoreBroken||null,wrapperGuard:!!window.__techopsWrapperGuardInstalled,wrapperHealth,hudFilter:window.TechOpsGoodBoysLegacyHudFilter&&window.TechOpsGoodBoysLegacyHudFilter.acceptance?window.TechOpsGoodBoysLegacyHudFilter.acceptance():null,runtimeLock:!!window.__productionFeatureWrapperTimersStopped,runtimeLockError:window.__productionRuntimeLockError||null,lexicalBridge:window.__techopsLexicalBridgeVersion||0,soundcloudSrc:sc&&sc.getAttribute('src')||null};
  });
}
...
async function settle(page,ms=5000){
  ...
  while(Date.now()<until){
    ...
    const nightCine=await page.evaluate(()=>!!(window.v722&&typeof window.v722.active==='function'&&window.v722.active())).catch(()=>false);
    if(nightCine){await page.evaluate(()=>window.v722.skip()).catch(()=>{});await page.waitForTimeout(180);continue;}
...
async function runMode(browserType,profileName,contextOptions,mode){
  ...
  page.on('console',m=>{if(m.type()==='error')consoleErrors.push(m.text());});page.on('pageerror',e=>pageErrors.push(String(e&&e.stack||e)));page.on('crash',()=>crashes.push({at:Date.now()}));page.on('requestfailed',r=>requestFailures.push({url:r.url(),error:r.failure()?.errorText||''}));page.on('response',r=>{if(r.status()>=400)badResponses.push({url:r.url(),status:r.status()});});
  try{
    ...
    await page.waitForTimeout(3000);const s2=await snapshotRuntime(page);repl(`${profileName} ${mode} liveness`,s2);if(crashes.length)fail(mode,'browser page crashed during liveness window',{profileName,crashes});if(s2.safetyError)fail(mode,'runtime errored during liveness window',{profileName,error:s2.safetyError});if(!s2.canvas.ok||(s2.canvas.nonBlack<20||s2.canvas.range<8))fail(mode,'canvas failed liveness window',{profileName,canvas:s2.canvas});
...
const profiles=[{id:'webkit',name:'iphone-webkit',browser:webkit,options:{...devices['iPhone 15 Pro'],viewport:{width:393,height:852}}},{id:'chromium',name:chromiumMobile?'iphone-chromium':'desktop-chromium',browser:chromium,options:chromiumMobile?{...devices['iPhone 15 Pro'],viewport:{width:393,height:852}}:{viewport:{width:1440,height:900}}}].filter(p=>enabledBrowsers.has(p.id));
