...
try{
  ...
  await dismiss(page,14000);let s=await state(page);log('started',s);if(!s.opening||!s.opening.completed||s.opening.count!==3){fail('opening-three-system-inspection-not-complete',s);throw new Error('opening did not complete; downstream progression assertions suppressed');}if(!s.active||!s.authority||!s.campaignState||!s.accessCore||!s.earthfall)fail('campaign-authorities-not-attached',s);if(s.followTrail)fail('retired-follow-trail-returned',s);if(s.mission!==2||s.metaMission!==2||s.stateMission!==2)fail('opening-did-not-canonically-enter-m2',s);if(s.acceptance&&s.acceptance.invariant&&s.acceptance.invariant.ok===false)fail('opening-handoff-invariant-failed',s.acceptance);
  ...
  for(let m=3;m<=7;m++){
    ...
    if(m===5){await page.waitForTimeout(350);cur=await state(page);log('access-core-roster',{kinds:cur.enemyKinds,access:cur.accessAcceptance});if(cur.enemyKinds.some(k=>String(k).toLowerCase().replace(/[\s_-]+/g,'').includes('mikeindex')))fail('m5-legacy-mike-index-still-active',cur);await page.evaluate(()=>{var n=window.NM,q=window.TechOpsGoodBoysAccessCoreAuthority;n.enemies.push({kind:'mikeindex',name:'THE MIKE INDEX',x:n.x+80,y:n.y,w:30,h:40,hp:99,maxHp:99,alive:true,boss:true});if(q)q.tick();});await page.waitForTimeout(120);cur=await state(page);if(cur.enemyKinds.some(k=>String(k).toLowerCase().replace(/[\s_-]+/g,'').includes('mikeindex'))||Number(cur.accessAcceptance&&cur.accessAcceptance.legacyMikeCount)>0)fail('m5-legacy-mike-index-respawn-not-quarantined',cur);}
    const primed=await page.evaluate(()=>window.TechOpsGoodBoysProgressionAuthority&&window.TechOpsGoodBoysProgressionAuthority.testPrimeClear());log('prime-clear',{mission:m,primed});if(!primed){fail('could-not-prime-clear',{mission:m});break;}await dismiss(page,8000);const next=m+1;cur=await waitMission(page,next,7000);log('mission-advance',{from:m,to:next,state:cur});if(!cur.done&&cur.mission!==next&&cur.metaMission!==next){fail('mission-did-not-advance',{from:m,to:next,state:cur});break;}if(m===4&&!cur.k)fail('k-not-unlocked-after-cell-118',cur);if(m===6&&!cur.waldo)fail('waldo-not-unlocked-after-cell-1984',cur);if(m<7)await dismiss(page,6000);
  }
