...
(function(root){
  ...
  var VERSION=3,running=false,replaying=false,complete=false,scene=0,overlay=null,canvas=null,ctx=null,raf=0,img=null,lastFrame="",timer=null,inputAttached=false;
  var SCENES=[
    ...
    {title:"GOOD DOGS PROTOCOL — COMPLETE",kicker:"118 FREE · 1984 FREE",body:"K walks out under his own name. Waldo walks back through his own door. Katrin and Manchez do what they came to do: bring everyone home.",goal:"BREAKOUT COMPLETE · KATRIN + MANCHEZ UNLOCKED",frames:["sh_wreck"],cta:"SAVE & RETURN TO TITLE"}
  ];
  ...
  function save(){try{if(typeof root.save==="function")return root.save()!==false;if(typeof root.saveGame==="function")return root.saveGame()!==false;return false;}catch(e){root.__goodBoysEarthfallSaveError=String(e&&e.stack||e);return false;} }
  ...
  function render(){
    if(!overlay)return;var s=SCENES[scene];var card=overlay.querySelector(".gbe-card");if(!card)return;card.innerHTML='<div class="gbe-kicker">GOOD DOGS PROTOCOL · '+s.kicker+' · '+(scene+1)+' / '+SCENES.length+'</div><h2>'+s.title+'</h2><p>'+s.body+'</p><div class="gbe-goal">'+s.goal+'</div><button id="gbe-next">'+(s.cta||(scene===SCENES.length-1?'END OPERATION':'CONTINUE'))+'</button>';var b=card.querySelector("#gbe-next");if(b)b.addEventListener("click",next);
  }
  function finishFlags(){
    try{var m=meta();if(!m)return false;m.k=true;m.waldo=true;m.done=true;m.m=8;root.S.meta._v736breakout=true;root.S.meta._v736pair=true;try{root.localStorage&&root.localStorage.setItem("techops_char_1181984","katrin_manchez");}catch(_){}var c=cs();if(c){c.m=8;c.ending=true;}if(root.NM){root.NM.enemies=[];root.NM.clear=false;root.NM._gbEarthfallComplete=true;}complete=true;root.__goodBoysEarthfallCompleteAt=Date.now();try{var semantic=root.TechOpsGoodDogsCampaignState;if(semantic&&typeof semantic.completeReturn==="function")semantic.completeReturn();}catch(e){root.__goodBoysSemanticCompletionError=String(e&&e.stack||e);return false;}if(!save())return false;return true;}catch(e){root.__goodBoysEarthfallCompleteError=String(e&&e.stack||e);return false;}
  }
  ...
  function close(){try{if(raf&&root.cancelAnimationFrame)root.cancelAnimationFrame(raf);}catch(_){}raf=0;detachInput();try{if(overlay&&overlay.parentNode)overlay.parentNode.removeChild(overlay);}catch(_){}overlay=null;canvas=null;ctx=null;running=false;replaying=false;try{if(root.document&&root.document.body){root.document.body.classList.remove("good-boys-earthfall-active");root.document.body.classList.remove("good-boys-cinematic");}if(root.S)root.S.inDialog=false;}catch(_){} }
  function returnToTitle(){
    root.__goodBoysEarthfallReturnToTitle={at:Date.now(),saved:complete};
    try{if(root.location&&typeof root.location.reload==="function"){root.location.reload();return true;}}catch(e){root.__goodBoysEarthfallReloadError=String(e&&e.stack||e);}
    try{var d=root.document,title=d&&d.getElementById("title-screen"),hud=d&&d.getElementById("hud"),touch=d&&d.getElementById("touch-ui");if(title)title.classList.remove("hidden");if(hud)hud.classList.add("hidden");if(touch)touch.classList.add("hidden");return!!title;}catch(e){root.__goodBoysEarthfallReturnError=String(e&&e.stack||e);return false;}
  }
  function next(e){if(e){e.preventDefault();e.stopPropagation();}if(!running)return false;if(scene<SCENES.length-1){scene++;render();return true;}if(!finishFlags())return false;close();returnToTitle();return true;}
  ...
  function begin(options){
    try{
      options=options||{};var replay=options.replay===true,m=meta();if(!m||(m.done&&!replay)){complete=!!(m&&m.done);return false;}if(running)return true;if(!root.document||!root.document.body){if(!replay)finishFlags();return true;}installStyle();running=true;replaying=replay;scene=0;var c=cs();if(c)c.ending=true;if(root.NM){root.NM.enemies=[];root.NM.clear=false;}if(root.S)root.S.inDialog=true;root.document.body.classList.add("good-boys-earthfall-active");root.document.body.classList.add("good-boys-cinematic");overlay=root.document.createElement("div");overlay.id="good-boys-earthfall-cine";var bg=bgSrc();if(bg)overlay.style.backgroundImage='linear-gradient(to bottom,rgba(1,4,8,.05),rgba(1,4,8,.55)),url("'+String(bg).replace(/"/g,"%22")+'")';overlay.innerHTML='<div class="gbe-shade"></div><canvas aria-hidden="true"></canvas><div class="gbe-card"></div>';root.document.body.appendChild(overlay);canvas=overlay.querySelector("canvas");ctx=canvas&&canvas.getContext?canvas.getContext("2d"):null;attachInput();render();if(ctx&&root.requestAnimationFrame)raf=root.requestAnimationFrame(drawFrame);root.__goodBoysEarthfallStartedAt=Date.now();root.__goodBoysEarthfallReplay=replay;return true;
  ...
  function acceptance(){var m=meta();return{version:VERSION,running:running,replaying:replaying,scene:scene+1,sceneCount:SCENES.length,lastFrame:lastFrame,mission:mission(),done:!!(m&&m.done),k:!!(m&&m.k),waldo:!!(m&&m.waldo),breakout:!!(root.S&&root.S.meta&&root.S.meta._v736breakout),pair:!!(root.S&&root.S.meta&&root.S.meta._v736pair),overlay:!!(root.document&&root.document.getElementById("good-boys-earthfall-cine")),complete:complete,terminalMissionNormalized:!!root.__goodBoysTerminalMissionNormalized};}
  timer=root.setInterval?root.setInterval(tick,90):null;
  root.TechOpsGoodBoysEarthfallEnding={VERSION:VERSION,SCENES:SCENES,begin:begin,replay:function(){return begin({replay:true});},next:next,finishFlags:finishFlags,returnToTitle:returnToTitle,tick:tick,acceptance:acceptance,detachInput:detachInput,timer:timer};
  tick();
