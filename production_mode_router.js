...
(function(root){
  ...
  function nightSnapshot(event,extra){
    var s=state(),n=world(),o={
      seq:++nightTraceSeq,
      at:(root.performance&&typeof root.performance.now==="function")?root.performance.now():(Date.now?Date.now():0),
      event:event||"snapshot",phase:nightLaunchPhase,desired:desired,
  ...
  function nightTrace(event,extra){
    var a=root.__productionNightLaunchTrace||(root.__productionNightLaunchTrace=[]),o=nightSnapshot(event,extra);
    a.push(o);if(a.length>160)a.splice(0,a.length-160);
    root.__productionNightLaunchPhase=nightLaunchPhase;
    return o;
  }
  ...
  function beginNightLaunch(){
    nightLaunchIssued=false;nightPollActive=false;nightCallbacks=[];nightTraceSeq=0;
    root.__productionNightLaunchTrace=[];root.__productionNightLaunchOk=false;
    setNightPhase("launch","launch.begin");
  }
  function finishNightLaunch(){
    clearBlockingDialog();forceNightIdentity();restoreRuntimeUi();clearErrors();
    desired=null;root.__productionDesiredMode=null;root.__productionNightLaunchOk=true;root.__productionActiveMode="nightcrawler";
    setNightPhase("ready","router.ready");
    var cbs=nightCallbacks.splice(0);for(var i=0;i<cbs.length;i++)try{cbs[i]();}catch(e){}
  }
  function failNightLaunch(reason,extra){
    root.__productionModeRouterError=reason||"night_runtime_timeout";
    setNightPhase("failed","launch.failed",extra||{});
    nightPollActive=false;nightCallbacks=[];
  }
  ...
  function issueNightEnter(){
    ...
    try{
      clearBlockingDialog();f();
      if(nightCinematicVisible())setNightPhase("cinematic","v722.begin");
      else if(nightRuntimeMounted())setNightPhase("mounting","enterNight.return",{mounted:true});
      else nightTrace("enterNight.return",{mounted:false});
      return true;
    }catch(e){
      root.__productionModeRouterEnterNightError=String(e&&e.stack||e);
      nightTrace("enterNight.error",{error:root.__productionModeRouterEnterNightError});
      return false;
    }
  }
  ...
  function enterNightReliably(done){
    ...
    if(wasCinematic){nightLaunchIssued=true;setNightPhase("cinematic","v722.observe");}
    else issueNightEnter();
    function poll(){
      var cine=nightCinematicVisible();
      if(cine){
        if(!wasCinematic)nightTrace("v722.begin");
        wasCinematic=true;nightLaunchIssued=true;nightLaunchPhase="cinematic";root.__productionNightLaunchPhase=nightLaunchPhase;
      }else if(wasCinematic){
        wasCinematic=false;setNightPhase("mounting","v722.end");
      }
      if(nightWorldReady()){
        nightPollActive=false;finishNightLaunch();return;
      }
      if(!cine&&!nightRuntimeMounted()&&!nightLaunchIssued)issueNightEnter();
      if(nightRuntimeMounted()&&!cine){
        clearBlockingDialog();
  ...
  function launchNightCrawler(){
    if((nightLaunchPhase==="launch"||nightLaunchPhase==="enter-requested"||nightLaunchPhase==="cinematic"||nightLaunchPhase==="mounting")&&desired==="nightcrawler"){
      nightTrace("launch.coalesced");enterNightReliably();return true;
    }
    ...
    try{
      ...
      if(!s&&start){
        setNightPhase("start-run","startRun.request");start();s=state();forceNightIdentity();
        nightTrace("startRun.return");
        if(nightCinematicVisible()){nightLaunchIssued=true;setNightPhase("cinematic","v722.begin");}
        else if(nightRuntimeMounted()){nightLaunchIssued=true;setNightPhase("mounting","runtime.attached");}
      }
    ...
    enterNightReliably();return true;
  }
  ...
  function launchGoodBoys(){
    ...
    function poll(){
      ...
      introAutomationAt=0;
      if(!nightRuntimeMounted()&&!priming){
        priming=true;
        var previousDesired=desired;desired="goodboys";
        enterNightReliably(function(){priming=false;desired=previousDesired||"goodboys";clearBlockingDialog();try{if(root.v736&&typeof root.v736.start==="function"&&!pairReady())root.v736.start();}catch(e){root.__productionModeRouterGoodBoysStartError=String(e&&e.stack||e);}poll();});
        return;
      }
  ...
  function captureIntent(ev){
    ...
    if(t.indexOf("NIGHT CRAWLER")>=0){
      try{ev&&ev.preventDefault&&ev.preventDefault();ev&&ev.stopPropagation&&ev.stopPropagation();ev&&ev.stopImmediatePropagation&&ev.stopImmediatePropagation();}catch(e){}
      setDesired("nightcrawler");forceNightIdentity();nightTrace("intent.capture");
      (root.setTimeout||setTimeout)(launchNightCrawler,0);return;
    }
  ...
  function healthTick(){
    if(desired==="nightcrawler"){
      forceNightIdentity();hideLegacyShell();
      if(nightWorldReady()){if(nightLaunchPhase!=="ready")finishNightLaunch();}
      else enterNightReliably();
    }else if(desired==="goodboys"){
      hideLegacyShell();if(goodBoysCinematicVisible()){confirmAutomationIntro();return;}
      if(nightRuntimeMounted()&&pairReady())finishGoodBoys();
    }else if(root.__productionActiveMode==="nightcrawler"&&nightWorldReady()){
      clearBlockingDialog();clearErrors();
    }else if(root.__productionActiveMode==="goodboys"&&nightRuntimeMounted()&&pairReady()){
      clearBlockingDialog();clearErrors();
    }
  }
