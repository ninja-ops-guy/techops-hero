/* Good Boys Access Core authority v2.
 ...
 * and keys encounter seeding to the current _v736 state so retries are fresh.
 */
(function(root){
  "use strict";
  if(!root)return;
  ...
  var VERSION=2,timer=null,purges=0,seeds=0,lastPurge=0,lastState=null,resets=0;
  ...
  function acceptance(){var count=0;try{count=(root.NM&&root.NM.enemies||[]).filter(legacyMike).length;}catch(_){}return{version:VERSION,mission:mission(),legacyMikeCount:count,purges:purges,seeds:seeds,resets:resets,lastPurge:lastPurge,securitySeeded:!!(root.NM&&root.NM._gbAccessCoreSecuritySeeded),retryStateBound:cs()===lastState,pass:mission()!==5||count===0};}
  timer=root.setInterval?root.setInterval(tick,45):null;
  root.TechOpsGoodBoysAccessCoreAuthority={VERSION:VERSION,tick:tick,purge:purge,seedSecurity:seedSecurity,syncState:syncState,legacyMike:legacyMike,acceptance:acceptance,timer:timer};
  tick();
