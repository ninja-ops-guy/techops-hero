...
(function(root){
 ...
 function prepare(){if(loading||engine||failed||!base)return loading;loading=import(new URL('level.mjs',base).href).then(function(m){return m.createLevel({assetBase:base});}).then(function(e){engine=e;return e;}).catch(function(e){failed=true;root.__goodDogs3DError=String(e&&e.message||e);return null;});return loading;}
 ...
 root.TechOpsGoodDogs3D={eligible:eligible,prepare:prepare,draw:draw,setView:function(v){if(engine)engine.setView(v);},status:function(){return {ready:!!engine,failed:failed,active:lastActive,mission:5,ownsStory:false,ownsLoop:false};}};
