...
export function createSession(root=globalThis,saveData=null){
 ...
 const platforms=[[250,345,240],[590,345,240],[930,310,230],[1260,345,260]].map(([x,y,w])=>({x,y,w,h:12}));n.platforms=platforms;
 function event(type,detail={}){events.push({type,...detail});}
 function setPause(value){paused=!!value;root.S.inDialog=paused;n.vx=0;c.partner.vx=0;}
 ...
 function attack(player=1){if(!canAct()||c.chars[identity(player)].hp<=0||(player===1&&block))return false;const body=player===2?c.partner:n;if(player===2?p2attackCD>0:attackCD>0)return false;
  if(player===2)p2attackCD=.34;else attackCD=.34;attackPose=.18;
  let hit=false;for(const e of n.enemies){if(e.alive===false||e.hp<=0)continue;const dx=e.x-body.x;if(Math.abs(dx)<135&&dx*body.face>-25){e.hp=Math.max(0,e.hp-26);e.hitT=8;if(!e.hp)e.alive=false;hit=true;}}
  if(hit)c.sync=Math.min(100,c.sync+8);event(hit?'hit':'swing',{player});return hit;
 }
 function jump(player=1){if(!canAct()||c.chars[identity(player)].hp<=0)return;const b=player===2?c.partner:n;if((b.jumps||0)<3){b.vy=-10.5;b.onGround=false;b.jumps=(b.jumps||0)+1;event('jump');}}
 function dash(){if(!canAct()||c.chars[c.active].hp<=0||dashCD>0)return;dashCD=.75;n.dash=.15;invulnerable=.2;event('dash');}
 ...
 function use(player=1){if(!canAct()||c.chars[identity(player)].hp<=0)return false;const other=identity(player===1?2:1);if(c.chars[other].hp<=0&&Math.abs(n.x-c.partner.x)<110){c.chars[other].hp=48;event('revive');return true;}if(player===2)return false;
 ...
 function action(name,player=1){if(name==='attack')return attack(player);if(name==='jump')return jump(player);if(name==='dash')return dash();if(name==='swap')return swap();if(name==='use')return use(player);return false;}
 function physics(b,axis,dt){const f=dt*60;const oldFeet=b.y+b.h;if(axis){b.face=Math.sign(axis);b.vx=axis*3.4;}else b.vx*=Math.pow(.65,f);
  if(b.dash>0){b.dash-=dt;b.vx=b.face*10;}b.x=Math.max(95,Math.min(1535,b.x+b.vx*f));b.vy=Math.min((b.vy||0)+.48*f,13.5);b.y+=b.vy*f;b.onGround=false;
  let floor=430;for(const pl of platforms)if(b.vy>=0&&oldFeet<=pl.y+2&&b.y+b.h>=pl.y&&b.x+b.w>pl.x&&b.x<pl.x+pl.w)floor=Math.min(floor,pl.y);
  if(b.y+b.h>=floor&&b.vy>=0){b.y=floor-b.h;b.vy=0;b.onGround=true;b.jumps=0;}
 }
 ...
 function tick(dt,input={}){
  if(!canAct())return;dt=Math.min(.05,Math.max(0,dt));time+=dt;attackCD=Math.max(0,attackCD-dt);p2attackCD=Math.max(0,p2attackCD-dt);dashCD=Math.max(0,dashCD-dt);invulnerable=Math.max(0,invulnerable-dt);invulnerable2=Math.max(0,invulnerable2-dt);attackPose=Math.max(0,attackPose-dt);block=!!input.block;
  if(c.chars[c.active].hp>0)physics(n,block?0:(input.axis||0),dt);else n.vx=0;
  const p=c.partner;
  if(c.chars[identity(2)].hp<=0)p.vx=0;else if(localCoop)physics(p,input.partnerAxis||0,dt);
  else{const delta=n.x-55-p.x;physics(p,Math.abs(delta)>35?Math.sign(delta):0,dt);if(p.onGround&&n.y+20<p.y){p.vy=-10.5;p.onGround=false;p.jumps=1;}}
  ...
  sampleCD-=dt;if(sampleCD<=0){sampleCD=.25;history.push({time,x:n.x});history=history.filter(s=>s.time>=time-4);}
  for(const e of n.enemies){if(e.alive===false||e.hp<=0)continue;e.hitT=Math.max(0,(e.hitT||0)-dt*60);e.cooldown=(e.cooldown??1.4)-dt;
   if(e.windup>0){e.windup-=dt;if(e.windup<=0){if(Math.abs(n.x-(e.recordedTarget??e.x))<75)damage(e.kind==='mikeindex'?20:12);if(Math.abs(p.x-(e.recordedTarget??e.x))<75)damage(e.kind==='mikeindex'?20:12,2);e.cooldown=e.kind==='mikeindex'?1.6:1.1;event('enemy-strike');}}
   else if(e.cooldown<=0){const prior=history.filter(s=>s.time<=time-.9).at(-1);e.recordedTarget=prior?prior.x:e.x;e.windup=.9;event('warning');}
   else if(Math.abs(e.x-n.x)>100)e.x+=Math.sign(n.x-e.x)*dt*(e.kind==='mikeindex'?48:62);
  }
  if(n.onGround&&n.x>845&&n.x<890)damage(8);
 ...
 function snapshot(){return {schema:1,complete,localCoop,time,active:c.active,x:n.x,y:n.y,partner:{x:c.partner.x,y:c.partner.y},chars:c.chars,indexDefeated:!!n._gbMikeIndexDefeated,securitySeeded:!!n._gbAccessCoreSecuritySeeded,nodeSeized:!!c._gbAccessNodeSeized,enemies:n.enemies.map(e=>({...e})),facts:{...facts}};}
 // Resume is applied after canonical authority has initialized this NM instance.
 function restore(data){if(!data||data.schema!==1||!Array.isArray(data.enemies)||!Number.isFinite(data.x))return false;
 ...
 return {root,n,c,action,tick,snapshot,restore,setPause,setCoop(v){localCoop=!!v;},get paused(){return paused;},get complete(){return complete;},get localCoop(){return localCoop;},get time(){return time;},drainEvents(){const e=events;events=[];return e;}};
}
