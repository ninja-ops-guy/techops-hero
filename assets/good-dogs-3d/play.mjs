import {createLevel} from './level.mjs?v=20260913-guided-r1';
import {movementInput} from './camera.mjs?v=20260913-guided-r1';
import {createSession} from './session.mjs?v=20260913-guided-r1';
...
new ResizeObserver(()=>{document.body.style.setProperty('--hud-bottom',Math.max(108,document.querySelector('header').getBoundingClientRect().bottom)+'px');}).observe(document.querySelector('header'));
...
function act(name,player=1){if(!ready||!started||session.paused)return;if(name==='use'&&!session.action('use',player)){say(session.guidance().hint);return;}else if(name!=='use')session.action(name,player);}
...
$('help').onclick=()=>{if(!started)return;session.setPause(true);save();overlay('HOW TO PLAY','K follows you. Clear the corridor, unlock the console, then reach Route 1984.','BACK TO GAME');};
...
const movement=['KeyW','KeyS','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','KeyA','KeyD','KeyB','KeyI','KeyK','KeyJ','KeyL','KeyF'];
...
for(const button of document.querySelectorAll('[data-hold],[data-action]')){
 ...
 button.addEventListener('pointerdown',e=>{e.preventDefault();button.setPointerCapture(e.pointerId);button.classList.add('held');if(button.dataset.hold)pointers.set(e.pointerId,button.dataset.hold);if(button.dataset.action)act(button.dataset.action);});
...
function input(){return {...movementInput(held,pointers.values(),'third'),attack:held.has('KeyF')||[...pointers.values()].includes('attack')};}
function frame(now){const dt=last?Math.min(.25,(now-last)/1000):0;last=now;if(ready){
 ...
 const guide=session.guidance();$('objective').textContent=guide.step+' / 4 · '+guide.title;$('instruction').textContent=guide.hint;$('waypoint').textContent=guide.direction+' · '+guide.metres+' m';document.querySelector('[data-action=swap]').disabled=session.localCoop||session.c.chars[session.c.active==='katrin'?'manchez':'katrin'].hp<=0;const use=document.querySelector('[data-action=use]');use.textContent=guide.useLabel;use.disabled=!guide.canUse;use.setAttribute('aria-label',guide.useLabel);document.querySelector('[data-action=attack]').classList.toggle('ready',guide.attackReady);$('active').textContent=session.c.active.toUpperCase();$('health').value=session.c.chars[session.c.active].hp;$('partner-health').textContent='PARTNER '+session.c.chars[session.c.active==='katrin'?'manchez':'katrin'].hp;$('sync').textContent='SYNC '+session.c.sync+'%';
 const target=guide.enemy;$('enemy').hidden=!target;if(target){$('enemy-name').textContent=target.kind==='mikeindex'?'WHITE PROJECTION · MIKE INDEX':'ROUTE SECURITY';$('enemy-health').max=target.maxHp;$('enemy-health').value=target.hp;}
 for(const event of session.drainEvents()){
  sound(event.type);if(event.type==='swing'&&event.player===1)say('Move closer to an enemy to land a strike.',850);if(event.type==='revive')say('PARTNER REVIVED · STAY TOGETHER',2000);if(event.type==='index-defeated')say('PROJECTION DEFEATED · NOW CLEAR THE GUARDS',3000);
  if(event.type==='node'){say('ROUTE UNLOCKED · GO TO THE FAR DOOR',3000);save();}
  if(event.type==='warning')say('ORANGE CIRCLE — STEP OUT OR HOLD BLOCK',1000);
