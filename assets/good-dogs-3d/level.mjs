...
import {prisonEntry} from './prison-entry.mjs?v=20260913-video-r1';
import {positionCamera} from './camera.mjs?v=20260913-video-r1';
import {GLTFLoader} from './vendor/GLTFLoader.js';
import {surface,dressActors,loadSurfaceMaps,environmentMap,filmPipeline,QUALITY} from './fidelity.mjs?v=20260913-video-r1';
import {buildCorridor,beveledBox} from './corridor.mjs?v=20260913-video-r1';
...
export async function createLevel({canvas,assetBase=new URL('./',import.meta.url),reducedMotion=globalThis.matchMedia?.('(prefers-reduced-motion: reduce)').matches||false,quality=globalThis.matchMedia?.('(pointer:coarse)').matches?'balanced':'high'}={}){
 ...
 async function loadActors(tier){
  ...
  const promise=Promise.all(['dog.katrin','dog.manchez','char.k'].map(id=>loader.loadAsync(new URL('models/'+id+(tier==='high'?'':'.lite')+'.glb',assetBase).href))).then(loaded=>{
   const set={actors:{},mixers:[]};['katrin','manchez','k'].forEach((id,i)=>{const gltf=loaded[i],o=gltf.scene;dressActors(o,id,maps);const mixer=new THREE.AnimationMixer(o);for(const clip of gltf.animations)mixer.clipAction(clip).play();set.mixers.push({mixer,id});set.actors[id]=o;});actorSets.set(tier,set);return set;
 ...
 function draw(n,width,height,time=performance.now()){
  ...
  mixers.forEach(a=>{const body=a.id===active?n:a.id===other?p:null;const speed=body?Math.abs(body.vx||0):Math.abs(n.vx||0);const moving=speed>.15;if(moving)a.mixer.update(dt*Math.min(1.65,Math.max(.35,speed/3.4)));else a.mixer.setTime(0);});
  const present=new Set();let telegraph=null;
  for(const e of n.enemies||[]){if(e.alive===false||e.hp<=0)continue;present.add(e);let o=enemyMeshes.get(e);if(!o){o=enemyMesh(e);enemyMeshes.set(e,o);}o.position.set(0,Math.max(0,(430-(e.y||390)-(e.h||40))/60),e.x/60);o.rotation.y=Math.PI;o.scale.setScalar(Math.max(.7,(e.h||70)/78));o.userData.healthBar.scale.x=.7*Math.max(0,e.hp/(e.maxHp||e.hp));if(e.windup>0)telegraph=e;}
