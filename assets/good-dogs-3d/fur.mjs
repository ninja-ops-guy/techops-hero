...
export function groomFur(actor,id){
 ...
 const c=document.createElement('canvas');c.width=128;c.height=256;const ctx=c.getContext('2d');ctx.fillStyle='#000';ctx.fillRect(0,0,128,256);ctx.strokeStyle='rgba(255,255,255,.88)';ctx.lineCap='round';
 ...
 const map=new THREE.CanvasTexture(c);map.colorSpace=THREE.NoColorSpace;map.anisotropy=4;(actor.userData.detailTextures??=[]).push(map);
 ...
 for(const source of meshes){
  ...
  if(!total)continue;
  const count=Math.min(actor.userData.videoMatch?6500:2000,Math.max(100,Math.round(total*(actor.userData.videoMatch?7500:3300)))),pos=[],norm=[],uv=[],bones=[],boneWeights=[],indices=[];
  ...
  for(let i=0;i<count;i++){
   ...
   const face=point.y>.58&&point.z>.29;const len=(face?.004:.010)+random()*(face?.005:.016),width=.007+random()*.009;point.addScaledVector(normal,.0015);tip.copy(point).addScaledVector(normal,len*.80);tip.y-=len*.18;
  ...
  const mat=new THREE.MeshStandardMaterial({alphaMap:map,color:source.material.color,roughness:1,side:THREE.DoubleSide,transparent:true,depthWrite:false,opacity:.72,alphaTest:.045,metalness:0});
