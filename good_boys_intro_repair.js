/* TechOps Hero — Good Boys direct cinematic intro v7
 * Canon opening: ship interior/takeover clips 01 + 02 -> one premise card -> gameplay.
 * SKIP exits the whole opening, TAKE CONTROL is an authored blocker, and launch is single-flight.
 */
(function(root){
  "use strict";
  if(!root)return;
  ...
  var VERSION=7,launching=false,premise=null,installedButton=null,launchEpoch=0;
  ...
  function dismissLegacy(){try{["good-boys-story-cine","good-boys-premise"].forEach(function(id){var n=root.document&&root.document.getElementById(id);if(n)n.remove();});}catch(_){}try{if(root.document&&root.document.body)root.document.body.classList.remove("good-boys-cinematic");}catch(_){} }
  ...
  function showPremise(){
    return new Promise(function(resolve){
      ...
      root.document.body.appendChild(premise);var b=premise.querySelector("button");var done=false;var finish=function(e){if(done)return;done=true;if(e){e.preventDefault();e.stopPropagation();}try{premise.remove();}catch(_){}premise=null;resolve();};b.addEventListener("click",finish,{once:true});
    });
  }
  async function playOpening(){
    ...
    var first=await root.GoodDogsCutscenes.play("GD_CUT_01",{force:true,muted:true});
    root.__goodBoysOpeningClipResult=first||null;
    if(first&&first.skipped)return first;
    var second=await root.GoodDogsCutscenes.play("GD_CUT_02",{force:true,muted:true});
    root.__goodBoysOpeningClipResult=second||null;
  ...
  async function launch(e){
    ...
    root.setTimeout(function(){if(epoch!==launchEpoch)return;startCampaign();root.setTimeout(function(){verify(1,epoch);},350);},0);
    return true;
  }
  function install(){var b=button();if(!b)return false;bypass();if(installedButton===b||b.dataset.gbiRepairInstalled==="7")return true;if(installedButton)try{installedButton.removeEventListener("click",launch,true);}catch(_){}b.addEventListener("click",launch,true);b.dataset.gbiRepairInstalled="7";installedButton=b;return true;}
  var timer=root.setInterval(function(){if(install())root.clearInterval(timer);},50);install();
  root.TechOpsGoodBoysIntroRepair={VERSION:VERSION,launch:launch,install:install,playOpening:playOpening,startCampaign:startCampaign,get launching(){return launching;},get premise(){return premise;}};
